...
- [ ] Optionally update CodeAnalysis packages in `Directory.Packages.props` to versions supported by the new SDK's Roslyn compiler. See [Upgrading the .NET SDK Version](../../src/docs/guides/upgrading-sdk-version/README.md) guide for details.
...
- [ ] Update `.github/actions/setup-dotnet/action.yml` to the required SDK versions. If dual targeting frameworks, add both. Use the same version as in `global.json`:
  ```yaml
  dotnet-version: | 
    10.0.302
  ```
