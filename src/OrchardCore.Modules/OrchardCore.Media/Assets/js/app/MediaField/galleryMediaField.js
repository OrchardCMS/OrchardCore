...
Vue.component("mediaFieldGalleryContainer", {
  ...
  watch: {
    ...
    gridView: {
      handler() {
        console.log("gridView changed to: ", this.gridView);
        this.setLocalStorageState();
      },
    },
  },
  methods: {
        ...
        this.sortableInstance = Sortable.create(this.$refs.mediaContainer, {
          ...
          onUpdate: function (evt) {
            ...
            const selectedElements =
              self.$refs.mediaContainer.querySelectorAll(".sortable-selected");
    ...
    showMediaTextModal: function showMediaTextModal(media) {
      ...
      setTimeout(() => this.$emit("updatemediatext"), 300); // Hack: Delay to allow bootstrap and Vue to settle
    },
    showAnchorModal: function showAnchorModal(media) {
      this.$parent.$emit("selectMediaRequested", media);
      setTimeout(() => this.$emit("updateanchor"), 300);
    },
    updateSelectedCount() {
      this.selectedItemCount =
        this.$refs.mediaContainer.querySelectorAll(".sortable-selected").length;
    },
    getfontAwesomeClassNameForFileName:
      function getfontAwesomeClassNameForFilename(filename, thumbsize) {
