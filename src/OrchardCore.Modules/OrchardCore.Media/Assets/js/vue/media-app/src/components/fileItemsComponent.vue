...
/**
 * Returns true if the given file is in the selectedFiles array,
 * false otherwise.
 * The file is considered selected if the file.url property
 * is equal to the url property of one of the elements in the
 * selectedFiles array, after both URLs have been converted to
 * lowercase.
 * @param file the file to check
 * @returns true if the file is selected, false otherwise
 */
const isFileSelected = (file: IFileLibraryItemDto) => {
  let result = props.selectedFiles?.some((element) => {
    if (file.url) {
 ...
 */
const changeSort = (newSort: any) => { // eslint-disable-line @typescript-eslint/no-explicit-any
