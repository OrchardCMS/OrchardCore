...
const fileItem = props.files?.at(0);

const emit = defineEmits<{
  (e: 'confirm', viewModel: IConfirmViewModel): void
  (e: 'closed', viewModel?: IConfirmViewModel): void
}>()
</script>

<template>
  <VueFinalModal :focus-trap="false" v-model="showModal" :modal-id="modalName" class="flex justify-center items-center"
    content-class="flex flex-column max-w-xl mx-4 p-4 rounded-lg space-y-2">
    ...
    <slot></slot>
    <div class="file-wrapper mb-5" v-if="props.files?.length == 1">
      <div class="img-wrapper">
        ...
        <span class="uppercase file-ext text-white">{{ getFileExtension(fileItem?.filePath) }}</span>
      </div>
      <div class="font-bold text-lg ml-2">{{ fileItem?.name }}</div>
    </div>
    <div class="mt-3 flex flex-row justify-content-end">
      <button class="btn btn-light border border-secondary" @click="emit('closed')">
        {{ t.Cancel }}
      </button>
