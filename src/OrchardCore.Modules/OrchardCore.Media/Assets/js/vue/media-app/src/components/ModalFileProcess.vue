<template>
  <VueFinalModal :focus-trap="false" :teleport-to="teleportTo" v-model="showModal" :modal-id="modalName"
    :esc-to-close="false" :click-to-close="false" class="flex justify-center items-center"
    ...
    <div class="file-list">
      <template v-for="(fileEntry, index) in inputValues" v-bind:key="index">
        <div class="m-3 ml-0 w-full flex flex-column">
          <div class="file-wrapper mb-5">
            <div class="img-wrapper">
              ...
              <span class="uppercase file-ext text-white">{{ getFileExtension(fileEntry.file.name) }}</span>
            </div>
            <div class="font-bold text-lg ml-2">{{ fileEntry.file.name }}</div>
          </div>
        </div>
      </template>
    </div>
    <div class="mt-3 flex flex-row justify-content-end align-items-center">
      <div class="flex align-items-center">
        <button class="btn btn-light border border-secondary cancel" @click="emit('closed')">
          {{ t.Cancel }}
        </button>
        <button id="btn-submit" class="ml-2 btn btn-primary submit"
