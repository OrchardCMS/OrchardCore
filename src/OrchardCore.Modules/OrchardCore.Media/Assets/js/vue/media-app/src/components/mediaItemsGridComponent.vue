    ...
    <ol class="row media-items-grid" :style="'grid-template-columns: repeat(auto-fill, minmax(' + thumbSize + 'px, 1fr));'">
      <li v-for="media in filteredMediaItems" :key="media.name" class="media-item media-container-main-list-item card p-0"
        ...
        <div class="thumb-container">
          <img v-if="media.mime.startsWith('image')" :src="buildMediaUrl(media.url, thumbSize)" :data-mime="media.mime" />
          <fa-icon v-else :style="{height: thumbSize - 10, margin: 8}" icon="fa-regular fa-file" :data-mime="media.mime"></fa-icon>
        </div>
        ...
        <div class="card-footer">
          <a :href="media.url" target="_blank"
            class="btn btn-light btn-sm inline-media-button view-button"><fa-icon icon="fa-solid fa-download"></fa-icon></a>
          <a alt="{{ t.EditButton }}" href="javascript:void(0)" class="btn btn-light btn-sm inline-media-button edit-button"
            @click="() => openModal(media, 'rename')"><fa-icon icon="fa-solid fa-edit"></fa-icon>
            <ModalInputConfirm :t="t" :action-name="t.RenameMediaTitle" :modal-name="getModalName(media.name, 'rename')" :new-name="media.name"
