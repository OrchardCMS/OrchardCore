...
public class MediaApiController : Controller
{
    ...
    public async Task<ActionResult<PermittedStorageDto>> GetPermittedStorage()
    {
        ...
        return Ok(new PermittedStorageDto { Bytes = bytes, Text = text });
    }
    ...
    public async Task<ActionResult<DirectoryTreeNodeDto>> GetDirectoryTree()
    {
        ...
        var cached = await _directoryTreeCache.GetTreeAsync();
        return Ok(ToDto(cached));
    }

    private static DirectoryTreeNodeDto ToDto(Services.DirectoryTreeNode node)
    {
    ...
    public async Task<IActionResult> DeleteFolder(string path)
    {
        ...
        {
            return this.ApiBadRequestProblem(detail: S["Cannot delete a system folder"]);
        }
        ...
        _directoryTreeCache.Invalidate();
    ...
    public async Task<IActionResult> MoveMedia(string oldPath, string newPath)
    {
        ...
        var movedFile = await _mediaFileStore.GetFileInfoAsync(newPath);
        await PreCacheRemoteMediaAsync(movedFile);
        ...
        [FromServices] IAuthorizationService authorizationService)
    {
        ...
        if (IsSpecialFolder(path))
        {
            return this.ApiValidationProblem(detail: S["Cannot create folder inside a system folder"]);
        }
    ...
    private bool IsSpecialFolder(string path)
        => string.Equals(path, _mediaOptions.AssetsUsersFolder, StringComparison.OrdinalIgnoreCase)
        || string.Equals(path, _attachedMediaFieldFileService.MediaFieldsFolder, StringComparison.OrdinalIgnoreCase);

    private async Task PreCacheRemoteMediaAsync(IFileStoreEntry mediaFile)
    {
