...
public sealed class HtmlFieldSettingsDriver : ContentPartFieldDefinitionDisplayDriver<HtmlField>
{
    ...
    public HtmlFieldSettingsDriver(
        IAuthorizationService authorizationService,
        IHttpContextAccessor httpContextAccessor,
        IStringLocalizer<HtmlFieldSettingsDriver> localizer)
    {
    ...
    private bool HasPostedRenderLiquid()
    {
        var fieldName = string.IsNullOrEmpty(Prefix)
            ? nameof(HtmlFieldSettingsViewModel.RenderLiquid)
            : $"{Prefix}.{nameof(HtmlFieldSettingsViewModel.RenderLiquid)}";
