  ...
  if (element) {
    ...
    var vm = new Vue({
      ...
      mounted: function mounted() {
        ...
        this.searchBoxContainer = $(this.$el).children().last(); // If we're loading an existing content item, we may already have a user picker
      ...
      methods: {
        ...
        onSelect: function onSelect(selectedOption, id) {
          ...
          self.arrayOfUsers.push(selectedOption); // We don't want to show the search box if we are only allowing a single user 
        ...
        remove: function remove(user) {
          this.arrayOfUsers.splice(this.arrayOfUsers.indexOf(user), 1); // After removing a selected user, we always want to show the search box 
