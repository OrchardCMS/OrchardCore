...
namespace OrchardCore.ContentFields.Drivers
{
    public class ContentPickerFieldDisplayDriver : ContentFieldDisplayDriver<ContentPickerField>
    {
        ...
        public override async Task<IDisplayResult> UpdateAsync(ContentPickerField field, IUpdateModel updater, UpdateFieldEditorContext context)
        {
            ...
            {
                updater.ModelState.AddModelError(field, Prefix, f => f.ContentItemIds, S["The {0} field is required.", context.PartFieldDefinition.DisplayName()]);
            }

            if (!settings.Multiple && field.ContentItemIds.Length > 1)
            {
                updater.ModelState.AddModelError(field, Prefix, f => f.ContentItemIds, S["The {0} field cannot contain multiple items.", context.PartFieldDefinition.DisplayName()]);
            }
