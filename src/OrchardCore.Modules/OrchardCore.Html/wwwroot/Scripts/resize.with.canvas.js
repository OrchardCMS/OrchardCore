...
function ResizeWithCanvas() {
  ...
  this.ctx = null;
  this.resizeimg = null; //PRIVATE FUNCTION
  ...
  var updateCanvas = function updateCanvas(canvas, ctx, img, canvasWidth, canvasHeight) {
    //square in the angle
    shapesFilled[0].points = {
      x: -10,
      y: -10,
      width: 20,
      height: 20
    };
    shapesFilled[1].points = {
      x: canvasWidth - 10,
      y: -10,
      width: 20,
      height: 20
    };
    shapesFilled[2].points = {
      x: -10,
      y: canvasHeight - 10,
      width: 20,
      height: 20
    };
    shapesFilled[3].points = {
      x: canvasWidth - 10,
      y: canvasHeight - 10,
      width: 20,
      height: 20
    ...
    ctx.drawImage(img, 10, 10, canvasWidth - 20, canvasHeight - 20); //get the offset to change the mouse cursor 
  ...
  this.init = function () {
    ...
    window.onscroll = function (ev) {
      ...
      reOffset(_this.resizecanvas);
    };

    window.onresize = function (ev) {
      console.log("onresize offset");
      reOffset(_this.resizecanvas);
    };
  };
  ...
  this.reset = function () {
    ...
    if (this.resizeimg !== null) {
      ...
      this.resizeimg.height = this.resizecanvas.clientHeight - 20; //sostituisce il canvas con l'immagine
  ...
  this.setup = function (img, resizableopt) {
    ...
    if (this.resizecanvas.getContext) {
      ...
      this.ctx = this.resizecanvas.getContext('2d'); //sostituisce l'immagine con il canvas
      ...
      $(this.resizecanvas).resizable(resizableopt).on('mousedown', function (ev) {
        return ev.preventDefault();
      });
      var _ctx = this.ctx;
      $(this.resizecanvas).on("mousemove", function (e) {
        mouseX = parseInt(e.clientX - offsetX);
        mouseY = parseInt(e.clientY - offsetY); // Put your mousemove stuff here
  ...
  this.refresh = function () {
    ...
    if (this.resizecanvas.getContext) {
      ...
      this.resizecanvas.height = this.resizecanvas.clientHeight; //updateCanvas(this.resizecanvas, this.ctx, this.resizeimg, this.resizecanvas.clientWidth, this.resizecanvas.clientHeight);
