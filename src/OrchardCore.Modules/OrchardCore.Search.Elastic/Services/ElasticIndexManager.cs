...
namespace OrchardCore.Search.Elastic
{
    ...
    public class ElasticIndexManager : IDisposable
    {
        ...
        private async Task WriteAsync(string indexName, Action<IndexWriter> action, bool close = false)
        {
            if (!_writers.TryGetValue(indexName, out var writer))
            {
                var indexAnalyzer = await _elasticIndexSettingsService.LoadIndexAnalyzerAsync(indexName);
                lock (this)
                {
                    if (!_writers.TryGetValue(indexName, out writer))
                    {
                        var directory = CreateDirectory(indexName);
                        var analyzer = _elasticAnalyzerManager.CreateAnalyzer(indexAnalyzer);
                        var config = new IndexWriterConfig(ElasticSettings.DefaultVersion, analyzer)
                        {
