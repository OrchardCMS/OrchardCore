...
public class ContainedPartLocalizationHandler : ContentLocalizationPartHandlerBase<LocalizationPart>
{
    ...
    public override async Task LocalizingAsync(LocalizationContentContext context, LocalizationPart part)
    {
        ...
        {
            var list = await _session.QueryIndex<LocalizedContentItemIndex>(i => (i.Published || i.Latest) && i.ContentItemId == containedPart.ListContentItemId).FirstOrDefaultAsync().ConfigureAwait(false);
            var localizedList = await _session.QueryIndex<LocalizedContentItemIndex>(i => (i.Published || i.Latest) && i.LocalizationSet == list.LocalizationSet && i.Culture == context.Culture).FirstOrDefaultAsync().ConfigureAwait(false);
...
public class LocalizationContainedPartHandler : ContentPartHandler<LocalizationPart>
{
    ...
    public override async Task CreatingAsync(CreateContentContext context, LocalizationPart instance)
    {
        ...
        {
            var list = await _session.QueryIndex<LocalizedContentItemIndex>(i => (i.Published || i.Latest) && i.ContentItemId == containedPart.ListContentItemId).FirstOrDefaultAsync().ConfigureAwait(false);
            var localizedList = await _session.QueryIndex<LocalizedContentItemIndex>(i => (i.Published || i.Latest) && i.LocalizationSet == list.LocalizationSet && i.Culture == instance.Culture).FirstOrDefaultAsync().ConfigureAwait(false);
