...
namespace OrchardCore.Lists.Drivers
{
    public class ListPartDisplayDriver : ContentPartDisplayDriver<ListPart>
    {
        ...
        public ListPartDisplayDriver(IContentDefinitionManager contentDefinitionManager, IContainerService containerService, IUpdateModelAccessor updateModelAccessor)
        {
        ...
        public override IDisplayResult Edit(ListPart part, BuildPartEditorContext context)
        {
            ...
            return Combine(InitializeEditListPartNavigationAdmin(part, context, settings), InitializeEditListPartHeaderAdmin(part, context, settings));
        }
        ...
        {
            return Initialize<ListPartHeaderAdminViewModel>(
                    "ListPartHeaderAdmin",
                    async model =>
                    {
        ...
        {
            return Initialize<ListPartNavigationAdminViewModel>(
        ...
        {
            return Initialize<ContentItemViewModel>("ListPartSummaryAdmin", model => model.ContentItem = listPart.ContentItem).Location("SummaryAdmin", "Actions:4");
        }
        ...
        private ShapeResult InitializeDisplayListPartDetailAdminShape(ListPart listPart, BuildPartDisplayContext context)
        {
            return Initialize(
                    "ListPartDetailAdmin",
                    (Func<ListPartViewModel, ValueTask>)(
                        async model =>
                        {
                            ...
                            model.ContentItems = (
                                await _containerService.QueryContainedItemsAsync(listPart.ContentItem.ContentItemId, settings.EnableOrdering, pager, containedItemOptions)
                            ).ToArray();
        ...
        {
            return Initialize<ListPartViewModel>(
