...
public class ThemeTogglerService
{
    ...
    public ThemeTogglerService(IHttpContextAccessor httpContextAccessor, ISiteService siteService, ShellSettings shellSettings)
    {
    ...
    public async Task<string> CurrentTheme()
    {
        ...
        if (adminSettings.DisplayThemeToggler)
        {
            ...
            if (_httpContextAccessor.HttpContext.Request.Cookies.TryGetValue(cookieName, out var value) && !string.IsNullOrWhiteSpace(value))
            {
                return value;
            }
