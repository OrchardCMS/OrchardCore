...
public class TwitterClientMessageHandler : DelegatingHandler
{
    ...
    public async Task ConfigureOAuthAsync(HttpRequestMessage request)
    {
        ...
        var settings = container.As<TwitterAuthenticationSettings>();
        var protrector = _dataProtectionProvider.CreateProtector(TwitterConstants.Features.TwitterAuthentication);
