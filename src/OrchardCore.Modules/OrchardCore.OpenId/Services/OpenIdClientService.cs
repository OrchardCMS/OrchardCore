...
namespace OrchardCore.OpenId.Services
{
    public class OpenIdClientService : IOpenIdClientService
    {
        ...
        public OpenIdClientService(ISiteService siteService, IStringLocalizer<OpenIdClientService> stringLocalizer)
        {
        ...
        public Task<ImmutableArray<ValidationResult>> ValidateSettingsAsync(OpenIdClientSettings settings)
        {
            ...
            {
                results.Add(new ValidationResult(S["The authority cannot be null or empty."], new[] { nameof(settings.Authority) }));
            }
            ...
            {
                results.Add(new ValidationResult(S["The response type cannot be null or empty."], new[] { nameof(settings.ResponseType) }));
            }
            else if (
            ...
            {
                results.Add(new ValidationResult(S["Unknown response type ."], new[] { nameof(settings.ResponseType) }));
            }

            if (string.IsNullOrEmpty(settings.ResponseMode))
            {
                results.Add(new ValidationResult(S["The response mode cannot be null or empty."], new[] { nameof(settings.ResponseMode) }));
            }
            else if (
                settings.ResponseMode != OpenIdConnectResponseMode.FormPost
                && settings.ResponseMode != OpenIdConnectResponseMode.Fragment
                && settings.ResponseMode != OpenIdConnectResponseMode.Query
            )
            {
                results.Add(new ValidationResult(S["Unknown response mode."], new[] { nameof(settings.ResponseMode) }));
            }
