...
namespace OrchardCore.OpenId
{
    ...
    internal static class OpenIdApplicationExtensions
    {
        ...
        public static async Task UpdateDescriptorFromSettings(this IOpenIdApplicationManager _applicationManager, OpenIdApplicationSettings model, object application = null)
        {
            ...
            foreach (var uri in
                (from uri in model.PostLogoutRedirectUris?.Split(_separator, StringSplitOptions.RemoveEmptyEntries) ?? Array.Empty<string>()
            ...
            foreach (var uri in
               (from uri in model.RedirectUris?.Split(_separator, StringSplitOptions.RemoveEmptyEntries) ?? Array.Empty<string>()
                select new Uri(uri, UriKind.Absolute)))
            {
