...
public class ApiController : Controller
{
    ...
    public ApiController(IAuthorizationService authorizationService, YesSql.ISession session, IClock clock)
    {
    ...
    public async Task<IActionResult> Read(ReadNotificationViewModel viewModel)
    {
        ...
        var notification = await _session
            .Query<Notification, NotificationIndex>(
                x => x.NotificationId == viewModel.MessageId && x.UserId == CurrentUserId(),
        ...
        return Ok(new { messageId = viewModel.MessageId, updated, });
    }
