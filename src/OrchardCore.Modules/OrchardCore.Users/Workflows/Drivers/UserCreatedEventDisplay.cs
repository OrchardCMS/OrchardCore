...
namespace OrchardCore.Users.Workflows.Drivers
{
    public class UserCreatedEventDisplay : ActivityDisplayDriver<UserCreatedEvent, UserCreatedEventViewModel>
    {
        ...
        {
            return Combine(
                Shape($"{typeof(UserCreatedEvent).Name}_Fields_Thumbnail", new UserCreatedEventViewModel(activity)).Location("Thumbnail", "Content"),
                Factory($"{typeof(UserCreatedEvent).Name}_Fields_Design", ctx =>
                {
