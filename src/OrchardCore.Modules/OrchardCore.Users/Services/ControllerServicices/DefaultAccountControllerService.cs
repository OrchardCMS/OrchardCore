...
namespace OrchardCore.Users.Services.ControllerServicices;

public class DefaultAccountControllerService : IAccountControllerAbstraction
{
    ...
    public DefaultAccountControllerService(
    ...
    public AccountController BindController { get; set; }

    public IActionResult ChangePassword(string returnUrl = null)
    {
        BindController.ViewData["ReturnUrl"] = returnUrl;
        return BindController.View();
    }

    public async Task<IActionResult> ChangePassword(ChangePasswordViewModel model, string returnUrl = null)
    {
        if (BindController.TryValidateModel(model) && BindController.ModelState.IsValid)
        {
            var user = await _userService.GetAuthenticatedUserAsync(BindController.User);
            if (await _userService.ChangePasswordAsync(user, model.CurrentPassword, model.Password, (key, message) => BindController.ModelState.AddModelError(key, message)))
            {
                if (BindController.Url.IsLocalUrl(returnUrl))
                {
                    await _notifier.SuccessAsync(H["Your password has been changed successfully."]);
                    return BindController.Redirect(returnUrl, true);
                }
                else
                {
                    return BindController.Redirect(BindController.Url.Action("ChangePasswordConfirmation"));
                }
            }
        }

        return BindController.View(model);
    }
    ...
    public async Task<IActionResult> DefaultExternalLogin(string protectedToken, string returnUrl = null)
    {
        ...
        return BindController.RedirectToAction(nameof(Login));
    }

    public IActionResult ExternalLogin(string provider, string returnUrl = null)
    {
        // Request a redirect to the external login provider.
        var redirectUrl = BindController.Url.Action(nameof(ExternalLoginCallback), "Account", new { returnUrl });
        var properties = _signInManager.ConfigureExternalAuthenticationProperties(provider, redirectUrl);
        return BindController.Challenge(properties, provider);
    }
    ...
    public async Task<IActionResult> ExternalLoginCallback(string returnUrl = null, string remoteError = null)
    {
        if (remoteError != null)
        {
            ...
            BindController.ModelState.AddModelError("", S["An error occurred in external provider."]);
        ...
        if (iUser != null)
        {
            ...
            {
                await _accountEvents.InvokeAsync((e, user, modelState) => e.LoggingInAsync(user.UserName, (key, message) => modelState.AddModelError(key, message)), iUser, BindController.ModelState, _logger);
                ...
                {
                    BindController.ModelState.AddModelError(string.Empty, S["Invalid login attempt."]);
                }
            }
        }
        else
        {
            ...
            BindController.ViewData["ReturnUrl"] = returnUrl;
            BindController.ViewData["LoginProvider"] = info.LoginProvider;

            if (iUser != null)
            {
                if (iUser is User userToLink && registrationSettings.UsersMustValidateEmail && !userToLink.EmailConfirmed)
                {
                    return BindController.RedirectToAction("ConfirmEmailSent", new { Area = "OrchardCore.Users", Controller = "Registration", ReturnUrl = returnUrl });
                }

                // Link external login to an existing user
                BindController.ViewData["UserName"] = iUser.UserName;
                BindController.ViewData["Email"] = email;

                return BindController.View(nameof(LinkExternalLogin));
            }
            else
            {
                ...
                if (registrationSettings.UsersCanRegister == UserRegistrationType.NoRegistration)
                {
                    ...
                    BindController.ModelState.AddModelError("", message);
                }
                else
                {
                    ...
                    {
                        iUser = await BindController.RegisterUser(new RegisterViewModel()
                        {
                        ...
                        if (iUser != null)
                        {
                            ...
                            if (identityResult.Succeeded)
                            {
                                ...
                                if (iUser is User user)
                                {
                                    ...
                                    {
                                        return BindController.RedirectToAction("RegistrationPending", new { Area = "OrchardCore.Users", Controller = "Registration", ReturnUrl = returnUrl });
                                    }
                                }
                    ...
                    return BindController.View("RegisterExternalLogin", externalLoginViewModel);
                }
            }
        }
    ...
    {
        var user = await _userManager.GetUserAsync(BindController.User);
        if (user == null)
        {
            return BindController.Forbid();
        }
    ...
    public async Task<IActionResult> LinkExternalLogin(LinkExternalLoginViewModel model, string returnUrl = null)
    {
        ...
        if (info == null)
        {
            ...
            return BindController.NotFound();
        }
        ...
        await _accountEvents.InvokeAsync((e, model, modelState) => e.LoggingInAsync(user.UserName, (key, message) => modelState.AddModelError(key, message)), model, BindController.ModelState, _logger);
    ...
    public async Task<IActionResult> LinkLogin(string provider)
    {
        ...
        await BindController.HttpContext.SignOutAsync(IdentityConstants.ExternalScheme);

        // Request a redirect to the external login provider to link a login for the current user
        var redirectUrl = BindController.Url.Action(nameof(LinkLoginCallback));
        var properties = _signInManager.ConfigureExternalAuthenticationProperties(provider, redirectUrl, _userManager.GetUserId(BindController.User));
    ...
    public async Task<IActionResult> LinkLoginCallback()
    {
        ...
        if (user == null)
        {
            _logger.LogError("Unable to load user with ID '{UserId}'.", _userManager.GetUserId(BindController.User));
            return BindController.RedirectToAction(nameof(Login));
        }
        ...
        return BindController.RedirectToAction(nameof(ExternalLogins));
    }

    public async Task<IActionResult> Login(string returnUrl = null)
    {
        if (BindController.HttpContext.User?.Identity?.IsAuthenticated ?? false)
        {
            returnUrl = null;
        }
        // Clear the existing external cookie to ensure a clean login process
        ...
        if (loginSettings.UseExternalProviderIfOnlyOneDefined)
        {
            ...
            if (schemes.Count() == 1)
            {
                ...
                return BindController.RedirectToAction(nameof(DefaultExternalLogin), new { protectedToken, returnUrl });
            }
        }

        foreach (var errorMessage in BindController.TempData.Where(x => x.Key.StartsWith("error")).Select(x => x.Value.ToString()))
        {
            BindController.ModelState.AddModelError(string.Empty, errorMessage);
        }
        BindController.ViewData["ReturnUrl"] = returnUrl;
        return BindController.View();
    }
    ...
    public async Task<IActionResult> Login(LoginViewModel model, string returnUrl = null)
    {
        ...
        if (BindController.TryValidateModel(model) && BindController.ModelState.IsValid)
        {
            ...
            {
                BindController.ModelState.AddModelError("", S["Local login is disabled."]);
            }
            else
            {
                await _accountEvents.InvokeAsync((e, model, modelState) => e.LoggingInAsync(model.UserName, (key, message) => modelState.AddModelError(key, message)), model, BindController.ModelState, _logger);
                if (BindController.ModelState.IsValid)
                {
                    ...
                    if (user != null)
                    {
                        ...
                        {
                            BindController.ModelState.AddModelError(string.Empty, S["The account is locked out"]);
    ...
    public async Task<IActionResult> LogOff()
    {
        ...
        return BindController.Redirect("~/");
    }
    ...
    public async Task<IActionResult> RegisterExternalLogin(RegisterExternalLoginViewModel model, string returnUrl = null)
    {
        ...
        BindController.ModelState.Clear();
        ...
        if (BindController.TryValidateModel(model) && BindController.ModelState.IsValid)
        {
            iUser = await BindController.RegisterUser(new RegisterViewModel() { UserName = model.UserName, Email = model.Email, Password = model.Password, ConfirmPassword = model.ConfirmPassword }, S["Confirm your account"], _logger);
            if (iUser is null)
            {
                BindController.ModelState.AddModelError(string.Empty, "Registration Failed.");
            }
            else
            {
                var identityResult = await _signInManager.UserManager.AddLoginAsync(iUser, new UserLoginInfo(info.LoginProvider, info.ProviderKey, info.ProviderDisplayName));
        ...
        return BindController.View("RegisterExternalLogin", model);
    }
    ...
    {
        var workflowManager = BindController.HttpContext.RequestServices.GetService<IWorkflowManager>();
    ...
    private async Task<bool> AddConfirmEmailError(IUser user)
    {
        ...
        if (registrationSettings.UsersMustValidateEmail == true)
        {
            ...
            {
                BindController.ModelState.AddModelError(string.Empty, S["You must confirm your email."]);
                return true;
            }
        }
    ...
    private bool AddUserEnabledError(IUser user)
    {
        ...
        {
            BindController.ModelState.AddModelError(String.Empty, S["The specified user is not allowed to sign in."]);
            return true;
        }
    ...
    private void AddIdentityErrors(IdentityResult result)
    {
        ...
        {
            BindController.ModelState.AddModelError(string.Empty, error.Description);
        }
    }
    ...
    private IActionResult RedirectToLocal(string returnUrl)
    {
        if (BindController.Url.IsLocalUrl(returnUrl))
        {
            return BindController.Redirect(returnUrl);
        }
        else
        {
            return BindController.Redirect("~/");
    ...
    {
        var iix = 0;
        foreach (var state in BindController.ModelState.Where(x => x.Key == string.Empty))
        {
            foreach (var item in state.Value.Errors)
            {
                BindController.TempData[$"error_{iix++}"] = item.ErrorMessage;
            }
        }
        return BindController.RedirectToAction(nameof(Login), new { returnUrl });
    }
