        ...
        public override void Describe(DescribeContext<ContentItem> context)
        {
            context.For<TaxonomyIndex>()
                .Map(contentItem =>
                {
                    ...
                    {
                        if (!contentItem.TryGetPropertyValue(fieldDefinition.PartDefinition.Name, out var node) ||
                            node is not JsonObject jPart ||
                            jPart[fieldDefinition.Name] is not JsonObject jField)
                        {
                            continue;
                        }

                        var field = jField.Deserialize<TaxonomyField>();
