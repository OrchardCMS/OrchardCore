...
public class Permissions : IPermissionProvider
{
    ...
    private static readonly IEnumerable<Permission> _allPermissions = [DemoAPIAccess, ManageOwnUserProfile,];

    private readonly IEnumerable<Permission> _generalPermissions = [ManageOwnUserProfile,];

    public Task<IEnumerable<Permission>> GetPermissionsAsync() => Task.FromResult(_allPermissions);

    public IEnumerable<PermissionStereotype> GetDefaultStereotypes() =>
        [
            new PermissionStereotype { Name = "Authenticated", Permissions = [DemoAPIAccess,], },
