...
namespace OrchardCore.Contents.AuditTrail.Services
{
    ...
    public class ContentAuditTrailEventDriver : AuditTrailEventDriverBase
    {
        private readonly ISession _session;
        private readonly IContentDefinitionManager _contentDefinitionManager;

        public ContentAuditTrailEventDriver(ISession session, IContentDefinitionManager contentDefinitionManager)
        {
            _session = session;
            _contentDefinitionManager = contentDefinitionManager;
        }

        public override async Task BuildViewModelAsync(AuditTrailEventViewModel viewModel)
        {
            // TODO: Seems to be done for each event !!!!
            if (viewModel.AuditTrailEvent.Category == "Content")
            {
                var shape = (IShape)viewModel;
                if (shape.Metadata.DisplayType == "Detail")
                {
                    await BuildDiffNodesAsync(viewModel);
                }

                var contentItem = viewModel.AuditTrailEvent.As<AuditTrailContentEvent>().ContentItem;
