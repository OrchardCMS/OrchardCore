    ...
    public class ContentMethodsProvider : IGlobalMethodProvider
    {
        ...
        public ContentMethodsProvider()
        {
            ...
            _createContentItemMethod = new GlobalMethod
            {
                ...
                Method = serviceProvider =>
                    (Func<string, bool?, object, IContent>)(
                        (contentType, publish, properties) =>
                        {
                            var contentManager = serviceProvider.GetRequiredService<IContentManager>();
                            var contentItem = contentManager.NewAsync(contentType).GetAwaiter().GetResult();
                            contentItem.Merge(properties);
                            var result = contentManager
                                .UpdateValidateAndCreateAsync(contentItem, publish == true ? VersionOptions.Published : VersionOptions.Draft)
                                .GetAwaiter()
                                .GetResult();
                            if (result.Succeeded)
                            {
