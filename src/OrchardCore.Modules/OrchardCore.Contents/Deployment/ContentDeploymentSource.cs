    ...
    : DeploymentSourceBase<ContentDeploymentStep>
{
    ...
    protected override async Task ProcessAsync(ContentDeploymentStep step, DeploymentPlanResult result)
    {
        ...
        foreach (var contentItem in await _session.Query<ContentItem, ContentItemIndex>(x => x.Published && x.ContentType.IsIn(step.ContentTypes)).ListAsync(CancellationToken.None))
        {
