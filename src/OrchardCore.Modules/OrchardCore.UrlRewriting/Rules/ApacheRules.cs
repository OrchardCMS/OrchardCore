...
public sealed class ApacheRules
{
    ...
    public static string FromViewModel(RewriteRuleViewModel viewModel, bool includeFlags = true)
    {
        ...
        var replaceUrl = viewModel.RuleAction == RuleAction.Rewrite
            ? viewModel.RewriteAction.RewriteUrl
            : viewModel.RedirectAction.RedirectUrl;

        var flagsStr = flags.Length > 0 ? $"[{flags}]" : "";
        if (flagsStr.Length > 0 && includeFlags)
        {
            return $"RewriteRule \"{viewModel.Pattern}\" \"{replaceUrl}\" {flagsStr}";
        }

        return $"RewriteRule \"{viewModel.Pattern}\" \"{replaceUrl}\"";
    }
