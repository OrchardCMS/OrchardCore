...
namespace OrchardCore.Tenants.Controllers
{
    public static class ControllerExtensions
    {
        public static async Task ValidateModelAsync<TViewModel>(this Controller controller, TViewModel model, bool newTenant = true) where TViewModel : TenantViewModel
        {
            ...
            var S = (IStringLocalizer)controller.HttpContext.RequestServices
                .GetService(typeof(IStringLocalizer<>).MakeGenericType(typeof(ControllerExtensions)));
            ...
            {
                controller.ModelState.AddModelError(nameof(TenantViewModel.ConnectionString), S["The connection string is mandatory for this provider."]);
            }

            if (String.IsNullOrWhiteSpace(model.Name))
            {
                controller.ModelState.AddModelError(nameof(TenantViewModel.Name), S["The tenant name is mandatory."]);
            }
            ...
            if (!String.IsNullOrWhiteSpace(model.FeatureProfile))
            {
                ...
                {
                    controller.ModelState.AddModelError(nameof(TenantViewModel.FeatureProfile), S["The feature profile does not exist.", model.FeatureProfile]);
                }
            }
            ...
            {
                controller.ModelState.AddModelError(nameof(TenantViewModel.Name), S["A tenant with the same name already exists.", model.Name]);
            }

            if (!String.IsNullOrEmpty(model.Name) && !Regex.IsMatch(model.Name, @"^\w+$"))
            {
                controller.ModelState.AddModelError(nameof(TenantViewModel.Name), S["Invalid tenant name. Must contain characters only and no spaces."]);
            }
            ...
            {
                controller.ModelState.AddModelError(nameof(TenantViewModel.RequestUrlPrefix), S["Host and url prefix can not be empty at the same time."]);
            }
            ...
            if (!String.IsNullOrWhiteSpace(model.RequestUrlPrefix))
            {
                ...
                {
                    controller.ModelState.AddModelError(nameof(TenantViewModel.RequestUrlPrefix), S["The url prefix can not contain more than one segment."]);
                }
            }
        }
