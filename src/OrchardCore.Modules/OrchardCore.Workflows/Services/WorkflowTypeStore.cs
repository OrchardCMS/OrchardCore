    ...
    public class WorkflowTypeStore : IWorkflowTypeStore
    {
        ...
        public async Task SaveAsync(WorkflowType workflowType)
        {
            ...
            if (isNew)
            {
                ...
                await _handlers.InvokeAsync((x, context) => x.CreatedAsync(context), context, _logger);
            }
            else
            {
                var context = new WorkflowTypeUpdatedContext(workflowType);
                await _handlers.InvokeAsync((x, context) => x.UpdatedAsync(context), context, _logger);
            }
        }
        ...
        public async Task DeleteAsync(WorkflowType workflowType)
        {
            ...
            await _handlers.InvokeAsync((x, context) => x.DeletedAsync(context), context, _logger);
        }
    }
}
