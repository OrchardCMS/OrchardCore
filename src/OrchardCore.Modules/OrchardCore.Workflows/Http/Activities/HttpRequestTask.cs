...
namespace OrchardCore.Workflows.Http.Activities
{
    public class HttpRequestTask : TaskActivity<HttpRequestTask>
    {
        ...
        private static readonly Dictionary<int, string> _httpStatusCodeDictionary =
            new()
            {
                { 100, "Continue" },
        ...
        public HttpRequestTask(IStringLocalizer<HttpRequestTask> localizer, IWorkflowExpressionEvaluator expressionEvaluator, UrlEncoder urlEncoder)
        {
            S = localizer;
            ...
            var outcomes = !string.IsNullOrWhiteSpace(HttpResponseCodes)
                ? HttpResponseCodes
                    ...
                    .Select(x =>
                    {
                        ...
                        var description = _httpStatusCodeDictionary.TryGetValue(status, out var text) ? $"{status} {text}" : status.ToString();
        ...
        private static IEnumerable<KeyValuePair<string, string>> ParseHeaders(string text)
        {
            ...
            return from header in text.Split(_separator, StringSplitOptions.RemoveEmptyEntries).Select(x => x.Trim())
        ...
        {
            return from code in text.Split(',', StringSplitOptions.RemoveEmptyEntries) select int.Parse(code);
        }
    }
}
