...
public class SecretsDocumentManager
{
    ...
    /// </summary>
    public Task<SecretsDocument> LoadSecretsAsync() => _documentManager.GetOrCreateMutableAsync();

    /// <summary>
    /// Returns the document from the cache or creates a new one. The result should not be updated.
    /// </summary>
    public Task<SecretsDocument> GetSecretsAsync() => _documentManager.GetOrCreateImmutableAsync();

    public async Task RemoveSecretAsync(string name)
    {
        var document = await LoadSecretsAsync();
