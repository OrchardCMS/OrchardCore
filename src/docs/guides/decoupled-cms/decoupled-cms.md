...
__Decoupled__ is a development model where the front-end and the back-end (administration) of the site are hosted in the same web application. Developers can then write their own ASP&#46;NET Razor Pages or Controllers to have total control on what is generated by the website.

// TODO Insert screenshot of the final app
...
In Orchard Core CMS most of the content that is managed in a __Content Item__. A content item is a versionned document like a page, and article, a blog post, a news item, or anything you want to manage. Each of these documents is based on a __Content Type__ that defines which properties it's made of. For instance any article will have a title and some text. A blog post might also have tags. Orchard Core CMS lets you model the content types the way you want, which is known as _content modeling_.

### Creating a Blog Post content type

Orchard comes pre-configured with a set of copmosable elements of data management called __Content Parts__ that can be used to create custom types like a LEGO. A __Title Part__ for instance will provide a nice editor to enter the title of a content item, and also set it to the text to display by default in the screens. Another important content part is the __Markdown Body Part__ which provides a way to store and render Markdown as the main text of a content item. This is also useful for a Blog Post.
...
- Select __`__Wysiwyg editor__`__ as the type of editor to use, then click __Save__:
...
This is dynamically extract the `DisplayText` property of a content item, in our case the __Title__, and call the `slugify` on this values, which will turn the title in a value that can be used in slugs.
...
- Edit content
- Define pages with custom routes
- Design dedicated templates
- Load content items from different identifiers
