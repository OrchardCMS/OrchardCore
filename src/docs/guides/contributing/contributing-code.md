...
- We're using [CodeRabbit](https://coderabbit.ai/) for AI code reviews, which they graciously provide for us as an open-source project for free. Please take the bot's feedback like a human's, and address them as well or dispute them by replying to the comments. You can have a conversation with it. Please be patient; AI code reviews help, but can frequently be incorrect. Use your best judgment. Note that if you open multiple pull requests within a few minutes, or push to an open one repeatedly, CodeRabbit may rate limit reviews; just wait as long as it says and ask it for a review with `@coderabbitai review`.
