...
Lastly, The method `IsRequiredAsync()` in both the `ITwoFactorAuthenticationHandlerCoordinator` and `ITwoFactorAuthenticationHandler` was changed to `IsRequiredAsync(IUser user)`.
