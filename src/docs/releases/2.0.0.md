...
The admin menu has been optimized with significant performance improvements and code enhancements. When adding a navigation provider to the admin menu, it is now recommended to use the new `AdminMenuNavigationProvider` class rather than directly implementing the `INavigationProvider` interface in your project.

By inheriting from `AdminMenuNavigationProvider`, you can choose to implement either the `Build(NavigationBuilder builder)` method for synchronous operations or the `BuildAsync(NavigationBuilder builder)` method for asynchronous operations, depending on your needs.
