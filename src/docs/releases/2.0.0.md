...
Previously, all custom queries were stored in the `QueriesDocument` using a `DocumentManager`. However, with [pull request #16402](https://github.com/OrchardCMS/OrchardCore/pull/16402), the `QueriesDocument` has been replaced with individual documents. This change involved modifications to the `IQueryManager`, `IQuerySource` interfaces, and `Query` class. Additionally, a new project, `OrchardCore.Queries.Core`, was introduced.
...
- **Removal of `QueriesDocument`**: Custom queries are now stored in individual documents instead of a single `QueriesDocument`.
