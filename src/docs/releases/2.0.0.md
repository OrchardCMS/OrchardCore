...
Previously, all custom queries were stored in `QueriesDocument` using a `DocumentManager`. However, [pull request #16402](https://github.com/OrchardCMS/OrchardCore/pull/16402) removed `QueriesDocument` and replaced it with individual documents. In the process, `IQueryManager` was removed, and the `Query` class along with `IQuerySource` interface was moved to a new project named `OrchardCore.Queries.Core`. A migration process was added to transition existing queries into the new structure, ensuring that this change does not impact existing tenants.
