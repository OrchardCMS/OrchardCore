...
The `IContentManager` interface has been updated. The method `Task<IEnumerable<ContentItem>> GetAsync(IEnumerable<string> contentItemIds, bool latest = false)` has been removed. Instead, use the method that accepts `VersionOptions`. You can specify either `VersionOptions.Latest` or `VersionOptions.Published`.
