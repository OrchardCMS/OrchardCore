...
Previously, all custom queries were stored in `QueriesDocument` using a `DocumentManager`. However, the [pull-request #16402](https://github.com/OrchardCMS/OrchardCore/pull/16402) removed `QueriesDocument` and replaced it with individual documents. In the process, `IQueryManager` was changed, while the `Query` class and the `IQuerySource` interface were moved to a new `OrchardCore.Queries.Core` project. A migration process was added to transition existing queries into the new structure, ensuring that this change does not impact existing tenants. In addition, we now request implementations of `IQuerySource` using keyed services. Here is an example of how we register new implementations:
