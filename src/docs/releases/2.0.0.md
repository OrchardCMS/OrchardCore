...
By inheriting from `AdminNavigationProvider`, you can choose to implement either the `Build(NavigationBuilder builder)` method for synchronous operations or the `BuildAsync(NavigationBuilder builder)` method for asynchronous operations, depending on your needs.
