...
#### Content Manager Methods Updated

Previously, the method
`Task<IEnumerable<ContentItem>> GetAsync(IEnumerable<string> contentItemIds, bool latest = false)`
in the `DefaultContentManager` class returned content items in the same order as the provided list of IDs. The new replacement method `GetAsync(IEnumerable<string> contentItemIds, VersionOptions options = null)` does **not** guaranteed to follow the input order. If you need the content items to be in the same order as the provided IDs, you can use the following approach:

```csharp
    var contentItemIds = new List<string> { "id1", "id2", "id3" };
    var contentItems = await contentManager.GetAsync(contentItemIds, options);

    var orderedItems = contentItems.OrderBy(ci => contentItemIds.IndexOf(ci.ContentItemId));
```
...
Additionally, the `Twilio` provider is no longer enabled by default. If you want to use the Twilio SMS provider, you must enable it on the SMS settings page under `Configuration` > `Settings` > `SMS` on the `Twilio` tab.
