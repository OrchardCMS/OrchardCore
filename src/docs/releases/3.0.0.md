...
To centeralize the user enabling and disabling functionality, the `IsEnabled` property has been removed from the `EditUserViewModel`. Instead, you can now use the `Enable` and `Disable` buttons in the **Users List** to manage user status, which utilizes both `EnableAsync()` and `DisableAsync()` that have been added into `IUserService`.

This change streamlines user management by consolidating these actions into a single interface.
