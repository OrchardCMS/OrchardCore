...
With the improvement done to the Indexing module, managing Azure AI Search indexes is not part of the indexing module. This change reduced the code complexity and simplify the user experience since we now have one UI for all indexes. Another advantage is that we are now able to add data sources other than contents.
...
```csharp
services.AddIndexingSource<CustomSourceIndexManager, CustomSourceDocumentManager, CustomSourceIndexNameProvider>(AzureAISearchConstants.ProviderName, "CustomSource", o =>
{
    o.DisplayName = S["Custom Source in Azure AI Search"];
    o.Description = S["Create an Azure AI Search index based on custom source."];
});
```  
...
- The following recipe steps were removed:
  - `azureai-index-create` instead you can use the new `Indexing` command.
  - `azureai-index-reset` instead you can use the new `ResetIndexing` command.
  - `azureai-index-rebuild` instead you can use the new `RebuildIndexing` command.
