...
Fixed [#17597](https://github.com/OrchardCMS/OrchardCore/issues/17597): when `ContainerName` was templated with Liquid (e.g. `{{ ShellSettings.Name }}_imagecache`), an underscore in the resolved name silently failed Azure's container naming rules and the container was never created. The resolved container name is now validated against Azure naming rules and a clear `Critical` log entry is emitted when invalid; the `RequestFailedException` returned by Azure for an invalid name is also surfaced with a dedicated error message pointing at the naming rules. In the same module, the `CreateContainer` configuration value is now correctly read from `OrchardCore_Media_Azure` (previously the class default `true` always won).
