...
The `PoParser` now becomes `static` class and supports both sync & async parsing. The `ParseAsync` method is the new async method for parsing PO files, while the `Parse` method is the new sync method.
