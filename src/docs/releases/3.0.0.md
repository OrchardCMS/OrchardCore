...
### IContentManager delete/cancel change

- The way content item deletion works in the IContentManager has been updated to support cancelable deletes. As part of this change, the relevant delete API signatures were modified:

RemoveAsync and related delete methods now return a bool indicating whether the deletion was actually performed or was cancelled by handlers (e.g. when a handler sets RemoveContentContext.Cancel = true). Previously these methods returned Task/Task<void> with no cancellation indication.

- Calling handlers (such as content handlers that may cancel a delete for reserved content types) should now check the bool return to know if a content item was actually removed.

This affects any code that directly calls these delete APIs or implements handlers around delete operations — you’ll need to update method signatures and handling accordingly.
...
// After: bool return
```csharp
bool removed = await contentManager.RemoveAsync(contentItem);
if (!removed) {
    // deletion has been cancelled by a handler
}
```
