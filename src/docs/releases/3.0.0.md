...
Upgrade to .NET 10.0 :
Orchard Core 3.0.0 requires .NET 10.0. Please ensure that your development environment and hosting infrastructure are compatible with this version of .NET before proceeding with the upgrade.
...
#### Liquid Slugify Filter
By default, Slugify string filter transliterates the string provided. The same applies to MediaSlugify module (if enabled) for Media files. 
#### Liquid Slugify Filter

By default, the `slugify` string filter now transliterates the string provided, i.e. it converts Unicode text (e.g. Greek, Cyrillic, Arabic, Chinese, etc.) into its closest ASCII/Latin representation. The same applies to the Media Slugify module (if enabled) for Media files.

There's also a new `transliterate` filter that just does transliteration.

### Media Module

#### ImageSharp replaced by NetVips as the default engine

On-demand image processing is no longer backed by `SixLabors.ImageSharp.Web`, and `SixLabors.ImageSharp` is no longer a dependency of the `OrchardCore.Media` module. Image resizing is now performed by a built-in middleware that delegates to a pluggable `IImageProcessingEngine` service. The default engine uses [NetVips](https://github.com/kleisauke/net-vips), a managed binding over the native [libvips](https://www.libvips.org/) library. This change was motivated by the move of ImageSharp to a commercial/split license, and brings significantly faster resizing through shrink-on-load decoding.

The native libvips binaries are provided automatically by the `NetVips.Native` package for the common runtimes. If you publish for a runtime that is not covered by the default native package, add the matching `NetVips.Native.<rid>` package to your web project.

#### Pluggable image processing engine

The engine contract (`IImageProcessingEngine`, `ImageProcessingCommands`, `ImageProcessingResult`) has been promoted to the public `OrchardCore.Media.Abstractions` assembly (namespace `OrchardCore.Media.Processing`). The middleware now parses the validated request into a typed, engine-agnostic `ImageProcessingCommands` instance, so engines no longer parse query strings, tokens or cache keys. The `ResizeMode` and `Format` enums (namespace `OrchardCore.Media.Core.Processing`) were also moved to `OrchardCore.Media.Abstractions`; type forwarders preserve binary compatibility for `OrchardCore.Media.Core` consumers, and the source namespace is unchanged.

If you previously implemented the internal engine contract, update your implementation to the public `IImageProcessingEngine` and replace the registration after the Media module is configured:

```csharp
services.Replace(ServiceDescriptor.Singleton<IImageProcessingEngine, MyImageProcessingEngine>());
```

#### Optional ImageSharp (v3) engine

For sites that want to keep ImageSharp-based processing, a new opt-in feature **Media ImageSharp Image Processing** (`OrchardCore.Media.ImageSharpV3`, in the package of the same name) provides an `IImageProcessingEngine` backed by `SixLabors.ImageSharp` `3.1.x`. Enabling the feature replaces the default NetVips engine. The `V3` moniker is intentional: ImageSharp v4 introduces build-time license-key enforcement and will be handled by a separate package.

#### Removed BMP and TGA support

libvips does not ship a BMP encoder or a TGA decoder, so the `bmp` and `tga` output formats are no longer supported. Requests that ask for these formats (via the `format` query argument or a media profile) now fall back to `jpeg`. The `Bmp` and `Tga` members of the `OrchardCore.Media.Core.Processing.Format` enum are kept for stored-profile compatibility but are marked `[Obsolete]` and removed from the media profile editor. The supported output formats are now `gif`, `jpg`, `png`, and `webp`.

#### Feature, configuration, and type renames

The ImageSharp-specific naming has been generalized across the Azure and Amazon S3 media cache features:

| Kind | Old | New |
| --- | --- | --- |
| Azure feature id | `OrchardCore.Media.Azure.ImageSharpImageCache` | `OrchardCore.Media.Azure.ImageCache` |
| Amazon S3 feature id | `OrchardCore.Media.AmazonS3.ImageSharpImageCache` | `OrchardCore.Media.AmazonS3.ImageCache` |
| Azure config section | `OrchardCore_Media_Azure_ImageSharp_Cache` | `OrchardCore_Media_Azure_Image_Cache` |
| Amazon S3 config section | `OrchardCore_Media_AmazonS3_ImageSharp_Cache` | `OrchardCore_Media_AmazonS3_Image_Cache` |
| Azure options type | `ImageSharpBlobImageCacheOptions` | `MediaBlobImageCacheOptions` |
| Amazon S3 options type | `AwsImageSharpImageCacheOptions` | `AwsMediaImageCacheOptions` |
| Configure order constant | `OrchardCoreConstants.ConfigureOrder.ImageSharpCache` | `OrchardCoreConstants.ConfigureOrder.ResizedImageCache` |
| Configure order constant | `OrchardCoreConstants.ConfigureOrder.AzureImageSharpCache` | `OrchardCoreConstants.ConfigureOrder.AzureResizedImageCache` |

If you enable these caches through a recipe, update the feature ids. If you configure them through `appsettings.json` or environment variables, rename the configuration sections accordingly.

To preserve compatibility, the legacy feature ids `OrchardCore.Media.Azure.ImageSharpImageCache` and `OrchardCore.Media.AmazonS3.ImageSharpImageCache` are still available and automatically enable the renamed features (`OrchardCore.Media.Azure.ImageCache` and `OrchardCore.Media.AmazonS3.ImageCache`) during upgrade, so a site that had the cache enabled before the rename keeps it enabled. The legacy ids are obsolete and only intended to ease the transition; update any recipes to the new ids.

#### `IResizedImageCache` API change

The cache abstraction no longer probes multiple candidate file extensions. The format of a resized image is now known up front, so `IResizedImageCache.GetAsync` takes the resolved file extension and performs a single deterministic lookup:
...
Custom `IResizedImageCache` implementations must be updated to the new signature.

#### Local cache folder renamed

The default filesystem cache for resized images previously stored files under `wwwroot/{tenant}/is-cache` (an ImageSharp-specific name). It now uses the generic `wwwroot/{tenant}/media-cache` folder. No action is required: the resized-image cache is fully derived from the source media and is repopulated on demand. After upgrading, the old `is-cache` folder under each tenant is no longer used and can be safely deleted to reclaim disk space.
...
The `PoParser` is now a `static` class and supports both synchronous and asynchronous parsing. The `ParseAsync` method is the new asynchronous method for parsing PO files, while the `Parse` method is the new synchronous method.
...
The `IEntity.As<T>()` and `IEntity.As<T>(string)` extension methods are now obsolete. Use `TryGet<T>()` for optional reads, or `GetOrCreate<T>()` when you want the previous create-if-missing behavior.
...
#### SMTP Pickup Directory Location

SMTP pickup directories are now sandboxed under a configurable `PickupDirectoryLocationBase`. The base path can only be changed through configuration, supports Fluid values such as `{{ AppData }}\Sites\{{ ShellSettings.Name }}\Emails`, and defaults to that tenant-specific `Emails` folder. `PickupDirectoryLocation` now defaults to `/`, which writes to the base folder itself.

This is a breaking change for configurations that previously used absolute paths in `PickupDirectoryLocation`. Invalid path characters and path navigation segments are also rejected. To preserve an external pickup folder, move the absolute directory to `PickupDirectoryLocationBase` and use `PickupDirectoryLocation: "/"` or a subfolder such as `"/Outbound"`.

#### Email Interfaces

The `IEmailService.SendAsync()` and `IEmailProvider.SendAsync()` methods now accept a `CancellationToken`.

The email event handler callbacks on `IEmailServiceEvents`, including overrides based on `EmailServiceEventsBase`, also now accept a `CancellationToken`.
...
- `TheAdminThemeOptions` class
- `Orchard.GetWrapperClasses()`, `Orchard.GetLabelClasses()`, `Orchard.GetEndClasses()`, `Orchard.GetStartClasses()`, `Orchard.GetOffsetClasses()`, `Orchard.GetLimitedWidthWrapperClasses()`, `Orchard.GetLimitedWidthClasses()` extension methods
- `Orchard.GetPartWrapperClasses()` and `Orchard.GetFieldWrapperClasses()` extension methods
- `TheAdminTheme:StyleSettings` configuration section

**Migration:**

Replace all calls to the removed helpers with static `ocat-*` CSS classes:

| Old Helper | New CSS Class(es) |
|---|---|
| `@Orchard.GetWrapperClasses()` | `ocat-wrapper` |
| `@Orchard.GetLabelClasses()` | `ocat-label` |
| `@Orchard.GetLabelClasses(inputRequired: true)` | `ocat-label ocat-label-required` |
| `@Orchard.GetEndClasses()` | `ocat-end` |
| `@Orchard.GetEndClasses(true)` | `ocat-end-offset` |
| `@Orchard.GetStartClasses()` | `ocat-start` |
| `@Orchard.GetOffsetClasses()` | `ocat-end-offset` |
| `@Orchard.GetLimitedWidthWrapperClasses()` | `ocat-limited-wrapper` |
| `@Orchard.GetLimitedWidthWrapperClasses("mb-3")` | `ocat-limited-wrapper` |
| `@Orchard.GetLimitedWidthClasses()` | `ocat-limited` |
| `@Orchard.GetPartWrapperClasses(typePartDefinition)` | `ocat-wrapper content-part-wrapper ...` |
| `@Orchard.GetFieldWrapperClasses(Model.PartFieldDefinition)` | `ocat-wrapper field-wrapper ...` |

**If you previously customized the layout via `appsettings.json`:**

Instead of configuring `TheAdminTheme:StyleSettings` in JSON or using `PostConfigure<TheAdminThemeOptions>`, you now override the `ocat-*` CSS classes in your custom admin theme's CSS. For example, to create a horizontal layout:

```css
.ocat-wrapper { --ocat-gutter-x: 1.5rem; display: flex; flex-wrap: wrap; margin-right: calc(-0.5 * var(--ocat-gutter-x)); margin-left: calc(-0.5 * var(--ocat-gutter-x)); margin-bottom: 1rem; }
.ocat-wrapper > *, .ocat-limited-wrapper > * { box-sizing: border-box; flex-shrink: 0; width: 100%; max-width: 100%; padding-right: calc(var(--ocat-gutter-x) * 0.5); padding-left: calc(var(--ocat-gutter-x) * 0.5); }
.ocat-label { flex: 0 0 auto; width: 25%; text-align: end; padding-top: calc(0.375rem + var(--bs-border-width)); }
.ocat-end { flex: 0 0 auto; width: 75%; }
.ocat-end-offset { flex: 0 0 auto; width: 75%; margin-inline-start: 25%; }
.ocat-start { flex: 0 0 auto; width: 25%; }
.ocat-limited-wrapper { --ocat-gutter-x: 1.5rem; display: flex; flex-wrap: wrap; margin-right: calc(-0.5 * var(--ocat-gutter-x)); margin-left: calc(-0.5 * var(--ocat-gutter-x)); margin-bottom: 1rem; }
.ocat-limited { flex: 0 0 auto; width: 33.333%; max-width: 33.333%; }
```

See the [Admin Theme Conventions](../reference/themes/admin-theme.md) documentation for complete details.
...
#### Feature and Project Renames

The Elasticsearch, Azure AI Search, and Lucene projects and primary feature IDs were renamed as follows:

- `OrchardCore.Search.Elasticsearch` -> `OrchardCore.Elasticsearch`
- `OrchardCore.Search.Elasticsearch.Core` -> `OrchardCore.Elasticsearch.Core`
- `OrchardCore.Search.AzureAI` -> `OrchardCore.AzureAI`
- `OrchardCore.Search.AzureAI.Core` -> `OrchardCore.AzureAI.Core`
- `OrchardCore.Search.Lucene` -> `OrchardCore.Lucene`
- `OrchardCore.Search.Lucene.Core` -> `OrchardCore.Lucene.Core`

To preserve compatibility, the legacy feature IDs `OrchardCore.Search.Elasticsearch`, `OrchardCore.Search.AzureAI`, and `OrchardCore.Search.Lucene` are still available and automatically enable the renamed features during upgrade.
...
Additionally, the `OrchardCore.Search.Elasticsearch.Abstractions` project has been removed, and the following classes have been relocated to the `OrchardCore.Elasticsearch.Core` project:
...
For more information, check out the [Elasticsearch documentation](../reference/modules/Elasticsearch/README.md#indexing-settings).
...
The `ISmsService.SendAsync()` and `ISmsProvider.SendAsync()` methods now accept a `CancellationToken`.

This is a source and binary breaking change for custom SMS services and providers. Update your implementations to include the new `cancellationToken` parameter.

### Notifications Module

The `INotificationService.SendAsync()` method now returns a `NotificationSendResult`, which provides the aggregate notification delivery outcome, including successful and failed sends across notification methods.

The `INotificationMethodProvider.TrySendAsync()` method was renamed to `SendAsync()`.

The `INotificationService.SendAsync()` and `INotificationMethodProvider.SendAsync()` methods now also accept a `CancellationToken`.

The notification event handler callbacks on `INotificationEvents`, including overrides based on `NotificationEventsHandler`, also now accept a `CancellationToken`.

This is a source and binary breaking change for custom notification services, method providers, and notification event handlers. Update your implementations and overrides to include the new `cancellationToken` parameter and handle `NotificationSendResult`.
...
#### Admin Menu Recipe Format

The `AdminMenu` format has changed. Admin menus already stored in the database are migrated automatically, but if you ship an `AdminMenu` recipe, you must update it to the new 3.0 format:
...
- Public static data migration methods are now supported for `Create`, `CreateAsync`, `UpdateFromX`, `UpdateFromXAsync`, `Uninstall`, and `UninstallAsync`, so CA1822 suppressions are no longer required for migration steps that don't use instance state.
...
- The `IAsyncOptions` interface is now obsolete and no longer supported. Use `IOptions` and synchronous options configuration instead.
- The `IAsyncConfigureOptions<TOptions>` interface is now obsolete and no longer supported. Use `IOptions` and synchronous options configuration instead.
- The obsolete `ServiceCollectionExtensions.Configure<TOptions>(Func<IServiceProvider, TOptions, ValueTask>)` extension method is no longer supported and will be removed in a future version. Use `Configure<TOptions>(Action<TOptions>)` instead.
- The obsolete `ServiceCollectionExtensions.Configure<TOptions, TConfigure>()` extension method is no longer supported and will be removed in a future version. Use `ConfigureOptions<TConfigureOptions>()` instead.
...
#### Database Presets, Required Prefixes, and Generated Database Names

Tenant creation and setup now better support shared-database and preset-database scenarios.

- A new `OrchardCore_Tenants:RequireTablePrefix` option can require a table prefix whenever the selected database provider supports table prefixes.
- New `OrchardCore_Tenants:TablePrefixPattern` and `OrchardCore_Tenants:SchemaPattern` options can generate the table prefix and schema automatically by using a Fluid template with access to `ShellSettings`, for example `{{ ShellSettings.Name }}`.
- Generated and manually entered table prefixes and schemas are now validated as SQL identifiers.

The tenant and setup UI behavior also changed when database settings are preset from configuration:

- If `OrchardCore:DatabaseProvider` is configured, the database provider is treated as fixed and is no longer selectable in the Setup or Tenants UI. The connection string is still shown when that provider requires one.
- If both `OrchardCore:DatabaseProvider` and `OrchardCore:ConnectionString` are configured, the connection string is also treated as fixed and is no longer shown or editable in the UI.
- In the tenant creation UI, the connection string is now optional so administrators can create a tenant and allow the tenant owner to provide the connection string later during setup.
- During tenant setup, Orchard Core now requires a connection string only when the selected provider needs one and the tenant doesn't already have a saved connection string.

Database validation and setup errors were also hardened so datastore failures return generic validation/setup messages instead of exposing raw database exception details in the UI.

For more details, see the [Configuration documentation](../reference/modules/Configuration/README.md) and the [Tenants documentation](../reference/modules/Tenants/README.md).
...
- Added export support for the Security module settings via a site settings property deployment step. See [#17418](https://github.com/OrchardCMS/OrchardCore/issues/17418).
...
- Added `IUserInfoClaimsProvider` to allow modules to append custom claims to the userinfo endpoint response.
...
### Content Transfer Module (New)

A new **Content Transfer** module has been added that provides bulk import and export of content items using Excel (`.xlsx`) files. Key features include:

- **Bulk Import**: Upload an Excel file to import content items. Files are processed in the background with configurable batch sizes, checkpoint-based resume, and progress tracking.
- **Bulk Export**: Export content items to an Excel file. Small datasets (≤500 records by default) are exported immediately; larger datasets are queued for background processing and the user is notified when the file is ready.
- **Export Dashboard**: A dedicated dashboard for managing queued export requests and downloading completed export files.
- **Export Filters**: Partial export support with filters for created/modified date ranges, owner usernames, published-only, latest-only, or all versions.
- **Validation**: Imported records are validated via `IContentManager.ValidateAsync()`. Rows that fail validation are tracked, and users can download an Excel file containing only the rejected records for review and re-import.
- **Notifications**: When the `OrchardCore.Notifications` module is enabled, users receive notifications when queued exports complete.
- **Extensible Handlers**: Custom import/export handlers can be added for any content part or content field. Built-in handlers are provided for `TitlePart`, `HtmlBodyPart`, `AutoroutePart`, and standard content fields (Text, Boolean, Number, Date, DateTime, Time, ContentPicker).

For more information, see the [Content Transfer documentation](../reference/modules/ContentTransfer/README.md).

### Data Orchestrator Module (New)

A new **Data Orchestrator** module has been added that provides a visual ETL (Extract, Transform, Load) pipeline editor for building data integration workflows. Key features include:

- **Extensible Sources**: Extract data from content items, a named query (SQL/Lucene/Elasticsearch via the Queries feature), an Excel workbook, or inline JSON. Add custom sources by deriving from `EtlSourceActivity`.
- **Transforms**: Reshape data with field mapping, filtering, value formatting, and data-set joins.
- **Extensible Destinations**: Publish the result to the media library (`Media Folder Export`) or to an FTP/FTPS server (`FtpExportLoad`, using FluentFTP), or create/update content items. Add custom destinations by deriving from `EtlFileExportLoad` (to reuse the export formats) or `EtlLoadActivity`.
- **Pluggable Export Formats**: File-based destinations share a registry of export formats — JSON, CSV, and Excel are built in, and new formats can be registered with `services.AddEtlExportFormat<T>()` so they become available to every destination.
- **Scheduling & Logs**: Pipelines can run manually or on a cron schedule via a background task, and each run is logged with status and processed/loaded record counts.

For more information, see the [Data Orchestrator documentation](../reference/modules/DataOrchestrator/README.md).

### Jint async scripting APIs

JavaScript global methods received the new `GlobalMethod.AsyncMethod` property, which is accessible in JavaScript as a function using the given name and the `Async` suffix, as you can see the example for the global method named `variables` below. This is non-breaking, but we suggest using the new methods where applicable going forward. In cases where the method value is immediately returned this is very simple, for example replacing `deleteContentItem` with `deleteContentItemAsync`:
...
which works without any need to `await`, because the expression returns a JavaScript `Promise` object, which is automatically awaited by the engine. Using the new version avoids a sync-over-async call that can cause a thread deadlock, so it's always preferable. Note that if you want to do operations on the result of an async method, you have to use [`.then()` chaining](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Using_promises#chaining) or wrap it in an [async function](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/async_function) so you can use the `await` keyword in the evaluated code.

## Miscellaneous

### Project Templates

Replace `wwwroot/.placeholder` files in project templates with MSBuild `MakeDir` target (#19028).
