...
A new `PlacementLocationBuilder` fluent API has been added as a type-safe alternative to manually constructing placement location strings in display drivers. The builder enforces the placement nesting hierarchy (**Zone → Tab → Card → Column**) through the type system, preventing misordering and making the placement intent explicit.
...
Each method in the chain returns a progressively narrower builder type, so only valid next steps are available. For example, after calling `.Tab()`, you can call `.Card()` or `.Column()`, but not `.Tab()` again. Levels can be skipped (e.g., `.Zone().Card()` without a `.Tab()` is valid).
