...
#### New `IContentDefinitionCoordinator` Interface

A new `IContentDefinitionCoordinator` interface has been introduced in `OrchardCore.ContentTypes.Abstractions`. This interface provides reusable coordinator methods for content definition operations that trigger events and can be used across modules without requiring a reference to the `OrchardCore.ContentTypes` module.
...
The existing `IContentDefinitionService` interface remains available for view model-related operations within the `OrchardCore.ContentTypes` module.
