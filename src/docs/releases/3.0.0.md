...
The `Localization` module now supports fallback to the parent culture when a translation is not available in the current culture. This means that if a specific translation is missing, the system will automatically look for the translation in the parent culture, providing a more seamless user experience.
