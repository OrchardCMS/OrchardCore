...
### Recipes and Deployment Module

#### Unified Recipe/Deployment Step Interface

A new unified interface `IRecipeDeploymentStep` has been introduced that combines:

- Recipe step execution (import)
- Deployment step generation (export)
- JSON Schema definition for validation and IntelliSense

**Benefits:**

- Single implementation handles both import and export.
- JSON Schema provides validation and editor IntelliSense.
- Reduced code duplication between recipe handlers and deployment sources.
...
|---------------|-------------|
| `IRecipeStepHandler` | `IRecipeDeploymentStep` |
| `NamedRecipeStepHandler` | `RecipeDeploymentStep<TModel>` |
| `AddRecipeExecutionStep<T>()` | `AddRecipeDeploymentStep<T>()` |

**Before (obsolete):**

```csharp
public class MyStep : NamedRecipeStepHandler
{
...
// Registration
services.AddRecipeExecutionStep<MyStep>();
```

**After (recommended):**

```csharp
public class MyStep : RecipeDeploymentStep<MyStep.StepModel>
{
    public override string Name => "MyStep";
    public override string DisplayName => "My Step";
    ...
    protected override Task ImportAsync(StepModel model, RecipeExecutionContext context)
    {
        // Handle import
        return Task.CompletedTask;
    }

    protected override Task<StepModel> BuildExportModelAsync(RecipeExportContext context)
    {
        // Handle export (return null to skip)
        return Task.FromResult<StepModel>(null);
    }

    public class StepModel { public string Name { get; set; } }
}

// Registration
services.AddRecipeDeploymentStep<MyStep>();
```

For import-only steps, use `RecipeImportStep<TModel>`.
For export-only steps, use `RecipeExportOnlyStep<TModel>`.

#### JSON Schema Validation in Import

The JSON Import feature now includes:

- Monaco editor with syntax highlighting.
- Real-time JSON Schema validation.
- IntelliSense with autocomplete for all registered recipe steps.

#### Content Definition Schema Handlers

New interfaces allow content parts and fields to contribute JSON Schema definitions for their settings:

```csharp
// For content part settings
public class MyPartSchemaHandler : IContentPartSchemaHandler
{
    ...
    public JsonSchema BuildSettingsSchema()
    {
        return new JsonSchemaBuilder()
            .Type(SchemaValueType.Object)
            .Properties(("Enabled", new JsonSchemaBuilder().Type(SchemaValueType.Boolean)))
            .Build();
    }
}

// For content field settings
public class MyFieldSchemaHandler : IContentFieldSchemaHandler
{
    ...
    public JsonSchema BuildSettingsSchema()
    {
        return new JsonSchemaBuilder()
            .Type(SchemaValueType.Object)
            .Properties(("Hint", new JsonSchemaBuilder().Type(SchemaValueType.String)))
            .Build();
    }
}
...
// Or when configuring content parts/fields
...
### Deployment: Automatic Deployment Steps from Recipe Steps

A new auto-discovery system has been introduced that **automatically generates deployment steps from recipe steps** that support export. This means any recipe step that extends `RecipeDeploymentStep<TModel>` with a `BuildExportModelAsync` implementation will automatically appear in the deployment plan UI — **no additional code is required**.

#### What Changed

Previously, adding a deployment step required creating **3 classes** (a `DeploymentStep`, a `DeploymentSourceBase<TStep>`, and a `DisplayDriver`) plus **3 Razor views** (Summary, Thumbnail, Edit), and registering them with `AddDeployment<>()`. Now, the `IDeploymentStepFactoryResolver` automatically discovers all `IRecipeDeploymentStep` implementations that support export and creates deployment step factories for them at runtime:

```csharp
// Before: Required 3 classes + 3 views + explicit registration for EACH deployment step.
services.AddDeployment<MyDeploymentSource, MyDeploymentStep, MyDeploymentStepDriver>();

// After: Just implement a recipe step with export support. No deployment code needed.
public sealed class MyRecipeStep : RecipeDeploymentStep<MyModel>
{
    public override string Name => "MyStep";

    protected override async Task<MyModel> BuildExportModelAsync(RecipeExportContext context)
    {
        return new MyModel { Items = await _myService.GetAllAsync() };
    }

    protected override Task ImportAsync(MyModel model, RecipeExecutionContext context)
    {
        return _myService.ImportAsync(model.Items);
    }
}
```
...
- `IDeploymentStepFactoryResolver` (new) combines explicitly registered `IDeploymentStepFactory` instances with auto-discovered recipe-step-based factories.
- At runtime, the resolver enumerates all `IRecipeDeploymentStep` implementations and filters to those that support export (i.e., not extending `RecipeImportStep<TModel>` which seals export to return null).
- For each exportable recipe step, a `RecipeStepDeploymentStepFactory` is created that produces `RecipeStepDeploymentStep` instances.
- `RecipeStepDeploymentSource` resolves the matching recipe step by name and calls `ExportAsync()`.
- `RecipeStepDeploymentStepDriver` renders generic views using metadata from `RecipeStepDeploymentOptions`.

#### Customizing Display Metadata

Auto-discovered deployment steps use the recipe step's `Name` property as the title by default. To provide localized titles and descriptions, use the optional `AddRecipeStepDeployment` extension method:

```csharp
// Optional: Configures localized display metadata for an auto-discovered deployment step.
services.AddRecipeStepDeployment<Startup>("MyStep", S => S["My Step"], S => S["Exports all my data."]);
```

#### Recipe Steps Upgraded to Support Export

The following built-in recipe steps have been upgraded from import-only (`RecipeImportStep`) to import+export (`RecipeDeploymentStep`) with `BuildExportModelAsync`. They now automatically appear as deployment steps:

| Module | Recipe Step | Recipe Step Name |
|--------|-----------|------------------|
| `OrchardCore.Roles` | `RolesRecipeStep` | `Roles` |
| `OrchardCore.Layers` | `LayerRecipeStep` | `Layers` |
| `OrchardCore.Sitemaps` | `SitemapsRecipeStep` | `Sitemaps` |
| `OrchardCore.Queries` | `QueryRecipeStep` | `Queries` |
| `OrchardCore.Templates` | `TemplateRecipeStep`, `AdminTemplateRecipeStep` | `Templates`, `AdminTemplates` |
| `OrchardCore.Placements` | `PlacementRecipeStep` | `Placements` |
| `OrchardCore.Shortcodes` | `ShortcodeTemplateRecipeStep` | `ShortcodeTemplates` |
| `OrchardCore.AdminMenu` | `AdminMenuRecipeStep` | `AdminMenu` |
| `OrchardCore.UrlRewriting` | `UrlRewritingRecipeStep` | `UrlRewriting` |
| `OrchardCore.Tenants` | `FeatureProfilesRecipeStep` | `FeatureProfiles` |
| `OrchardCore.Media` | `MediaProfileRecipeStep` | `media-profiles` |

Existing custom deployment steps (e.g., `AllContentDeploymentStep`, `ContentDeploymentStep`, `SiteSettingsDeploymentStep`, `AllWorkflowTypeDeploymentStep`, etc.) are preserved alongside the auto-discovered ones.

#### Breaking Changes

- `DeploymentPlanController`, `StepController`, and `DeploymentPlanDeploymentSource` now use `IDeploymentStepFactoryResolver` instead of `IEnumerable<IDeploymentStepFactory>`. If you injected the factory enumerable directly, switch to the resolver.

#### Migration Guide for Custom Deployment Steps

If you have **custom deployment steps** that export recipe JSON, you can simplify them:

1. **Change the recipe step base class** from `RecipeImportStep<TModel>` to `RecipeDeploymentStep<TModel>`.

2. **Implement `BuildExportModelAsync`** to return the model that produces the export JSON:
    ...
    public sealed class MyRecipeStep : RecipeDeploymentStep<MyModel>
    {
        ...
        public MyRecipeStep(IMyService myService)
        {
...
3. **That's it.** The deployment step is automatically discovered and registered. No `AddDeployment<>()`, no custom source/driver classes, no views needed.

4. **Optionally**, add localized display metadata:

    ```csharp
    services.AddRecipeStepDeployment<Startup>("MyStep", S => S["My Step"], S => S["Exports all my step data."]);
    ```

5. **Optionally**, remove old deployment classes (Source, Step, Driver, and Views) if they are now redundant.

Custom deployment steps with **special behavior** (e.g., selecting specific content types, rebuilding indexes, exporting files) should continue using the traditional `AddDeployment<>()` pattern.
