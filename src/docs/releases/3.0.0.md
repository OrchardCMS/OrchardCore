...
- If `OrchardCore:DatabaseProvider` is configured, the database provider is treated as fixed and is no longer selectable in the Setup or Tenants UI.
...
- If only `OrchardCore:DatabaseProvider` is configured, the provider remains fixed, but the connection string is still shown when that provider requires one.
