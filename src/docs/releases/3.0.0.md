...
By default, `Elasticsearch.Net` v9 expects your Elasticsearch server to be version 9 or higher. If your server is running version 8 or below, you can use the `CompatibleVersion` option in `ElasticsearchConnectionOptions` to specify the version of your Elasticsearch server.

This setting is important because different versions of the Elasticsearch client library are not always backward-compatible. By explicitly specifying the compatibility version, you ensure that the requests made by the client remain compatible with your server version.

You can configure this setting using any supported configuration provider. For example, if you're using `appsettings.json`, you can add the following section:
...
!!! note
    If your server is on version 9 but you need to maintain compatibility with version 8, setting `CompatibleVersion` to `"8"` will force the client to send requests compatible with Elasticsearch v8. This serves as a temporary workaround. However, the recommended long-term solution is to upgrade your server to v9 for full compatibility with the latest client features.
...
Now Elasticsearch module supports multiple index sources. When the **Contents** feature is enabled, the `Contents` source is automatically added, allowing you to create indexes based on content types.  
