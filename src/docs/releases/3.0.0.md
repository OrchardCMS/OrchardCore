...
The SMTP `PickupDirectoryLocation` setting now supports resolving `~/` from the configured `App_Data` folder and Fluid expressions such as `{{ ShellSettings.Name }}`. This makes it possible to use paths like `~/Emails`, `~/MyEmails/Etc`, or `~/Sites/{{ ShellSettings.Name }}/Emails` for tenant-aware pickup directories.
