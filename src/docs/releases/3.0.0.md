...
The user interface for managing indexes has been upgraded to support multiple index sources. When the **Contents** feature is enabled, `Contents` source is automatically added, allowing you to create indexes based on content types. We have also included a migration process to ensure backward compatibility.  

To add a new custom source using your own project, use the following configuration:  
...
Additionally, we've introduced a new event interface, `IAzureAISearchIndexSettingsHandler`, allowing you to customize index settings. You can implement this interface using `AzureAISearchIndexSettingsHandlerBase` for easier integration. We've also added the `IAzureAISearchEvents` interface.
