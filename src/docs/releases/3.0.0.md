...
The External Authentication feature previously raised events by calling methods on implementations of the `ILoginFormEvent` interface, despite not actually being part of the login form. While there was an `IExternalLoginEventHandler` interface, just used during external login, that actually contained methods that produced various values, rather than events.

Apart from all of this being inconsistent with the rest of the Users module, it also made it impossible to just target local or external logins with event handlers, causing [bug #17422](https://github.com/OrchardCMS/OrchardCore/issues/17422).

To fix this, we renamed the former `IExternalLoginEventHandler` to `IExternalLoginUserHandler`, but the behavior remained the same. Additionally, we have introduced a new `IExternalLoginEventHandler` interface that includes event handler methods just for external login events, and the external login process now doesn't raise `ILoginFormEvent` events.

!!! tip
    If you have a custom `ILoginFormEvent` implementation, note that it won't be called for external logins anymore. Implement `IExternalLoginEventHandler` instead, or additionally. If you have a custom `IExternalLoginEventHandler` implementation, you should change it to implement `IExternalLoginUserHandler` (no other changes necessary).
