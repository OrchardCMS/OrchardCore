...
The interface now uses DTOs instead of ViewModels for alter operations, making it more reusable:

- `AddTypeAsync` - Creates a new content type
- `RemoveTypeAsync` - Removes a content type
- `AddPartToTypeAsync` - Adds a part to a content type
- `AddReusablePartToTypeAsync` - Adds a reusable part with a custom name
- `RemovePartFromTypeAsync` - Removes a part from a content type
- `AddPartAsync` - Creates a new content part definition
- `RemovePartAsync` - Removes a content part definition
- `AddFieldToPartAsync` - Adds a field to a content part
- `RemoveFieldFromPartAsync` - Removes a field from a content part
- `AlterFieldAsync` - Alters a field's settings using `AlterFieldContext`
- `AlterTypePartAsync` - Alters a type part's settings using `AlterTypePartContext`
- `AlterTypePartsOrderAsync` - Alters the order of parts in a content type
- `AlterPartFieldsOrderAsync` - Alters the order of fields in a content part
- `GenerateContentTypeNameFromDisplayNameAsync` - Generates a unique content type name
- `GenerateFieldNameFromDisplayNameAsync` - Generates a unique field name

The `IContentDefinitionCoordinator` interface has been removed and replaced by `IContentDefinitionService`.

ViewModel-related methods (such as `LoadTypesAsync`, `GetTypesAsync`, `LoadTypeAsync`, `GetTypeAsync`, `LoadPartsAsync`, `GetPartsAsync`, `LoadPartAsync`, `GetPartAsync`, `GetFieldsAsync`) have been removed from the interface and are now private helpers within the `AdminController` in the `OrchardCore.ContentTypes` module.
