...
OrchardCore 3.0 introduces a significant change to how database transactions are committed during HTTP requests to fix race conditions.

**Problem:** Previously, database commits occurred during scope disposal (after the HTTP response was sent). This created a race condition where follow-up requests could fail because prior writes weren't yet durable. This particularly affected OpenID Connect flows where token creation would return HTTP 200, but an immediate `/userinfo` call would fail because the token wasn't committed yet.

**Solution:** Database commits now occur **before** the HTTP response is sent, using ASP.NET Core's `Response.OnStarting` callback. This ensures that all database changes are durable before the client receives a success response.

**Implementation:**
- When `ISession` or `IDocumentStore` is resolved during an HTTP request, a `Response.OnStarting` callback is automatically registered in the factory method
- The callback validates the current `ShellScope` hasn't changed and commits the session before the response starts
- `ISession` commits via `ISession.SaveChangesAsync()`
- `IDocumentStore` commits via `IDocumentStore.CommitAsync()`
- The session object itself is used as a key in `HttpContext.Items` to track whether a commit has occurred, providing type safety
- Commits run only once per request
- Commits are skipped if an exception occurs during request processing (tracked via the `OrchardCore:ExceptionOccurred` flag) to prevent committing potentially inconsistent state
- ShellScope validation ensures commits only happen in the correct scope

**Backward Compatibility:**
- For non-HTTP contexts (background jobs, shell initialization, independent ShellScopes), the original `BeforeDispose` commit mechanism is retained as a fallback
- The BeforeDispose callback checks if the commit was already performed via the `OnStarting` callback (by checking for the session key in `HttpContext.Items`) and only commits if needed
- This ensures proper behavior in all execution contexts while avoiding double-commits

**Impact:** This change eliminates race conditions where client requests could fail due to uncommitted data, particularly improving reliability for OAuth/OpenID Connect flows and other scenarios with immediate follow-up requests.
