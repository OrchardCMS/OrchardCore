...
```json
"ContentItemId": "[js: variablesAsync('blogContentItemId')]"
```

which works without any need to `await`, because the expression returns a JavaScript `Promise` object, which is automatically awaited by the engine. Using the new version avoids a sync-over-async ccall that can cause a thread deadlock, so it's always preferable. Note that if you want to do operations on the result of an async method, you have to use [`.then()` chaining](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Using_promises#chaining) or wrap it in an [async function](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/async_function) so you can use the `await` keyword in the evaluated code. For instance, the Blog recipe

```json
"LinkUrl": "[js: variables('adminUrlPrefix') + '/Contents/ContentItems/' +  variables('blogContentItemId') + '/Edit']",
```

is replaced with

```json
"LinkUrl": "[js: (async function() { return `${await variablesAsync('adminUrlPrefix')}/Contents/ContentItems/${await variablesAsync('blogContentItemId')}/Edit`; })()]",
```

because concatenating async calls with string would result in `/Admin/[object Promise]/Contents/ContentItems/[object Promise]/Edit`. 

Going forward if your own global function uses async code, we suggest only setting the `GlobalMethod.AsyncMethod`.
