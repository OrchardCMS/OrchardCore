...
## `IContentManager` Delete Behavior

`IContentManager` delete methods now support cancelable deletes. As part of this change:

- `RemoveAsync` and related APIs now return a `bool` indicating whether the delete actually occurred ('true') or was cancelled by a handler (`false`, e.g., when `RemoveContentContext.Cancel = true`).
- Previously these methods returned `Task/Task<void>` with no cancellation feedback.

### Impact:

Code that calls these delete APIs or implements handlers involved in delete operations must be updated to use the new `bool` return value and adjust method signatures accordingly.
