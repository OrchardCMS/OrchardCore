...
A new shape `SummaryAdminUserDisplayName` has been introduced to render a user's display name.

You can inject this shape anywhere you need the display name to appear, for example:

```liquid
<shape 
    Type="SummaryAdminUserDisplayName" 
    prop-userName="@(contentItem.Author)" 
    cache-id="user-display-name" 
    cache-context="user" 
    cache-tag="user-display-name,user-display-name:@contentItem.Author" />
```

This ensures consistent rendering of user names while taking advantage of Orchard's caching system for performance.
