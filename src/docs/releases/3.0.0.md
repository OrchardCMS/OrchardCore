...
On-demand image processing is no longer backed by `SixLabors.ImageSharp.Web` by default, and `SixLabors.ImageSharp` is no longer a dependency of the `OrchardCore.Media` module. Image resizing is now performed by a built-in middleware that delegates to a pluggable `IImageProcessingEngine` service. The default engine uses [NetVips](https://github.com/kleisauke/net-vips), a managed binding over the native [libvips](https://www.libvips.org/) library. This change was motivated by the fact that new versions of ImageSharp require a license key to build the software. That would have been an inconvenience to any contributor or users as part of building Orchard Core.
