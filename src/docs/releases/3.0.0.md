...
#### What changed
- New types:
  - `Result` — indicates success or failure and holds `IEnumerable<ResultError>`.
  - `Result<T>` — same, plus a `T` value for successful results.
  - `ResultError` — represents an error; messages support `LocalizedString`.
- Factory helpers:
  - `Result.Success()` and `Result.Success(value)`
  - `Result.Failed(params ResultError[])` and `Result.Failed(LocalizedString)`

#### Usage

1. Return a simple success or failed result:

```csharp
var successResult = Result.Success();
var failedResult = Result.Failed(new ResultError { Message = new LocalizedString("key", "message text") });
```

2. Return a successful result with a value:

```csharp
var successResultWithValue = Result.Success("value");
```

3. Return a failed result with multiple errors:

```csharp
var failedResultWithMultipleErrors = Result.Failed(
    new ResultError { Message = new LocalizedString("key1", "message text 1") },
    new ResultError { Message = new LocalizedString("key2", "message text 2") }
);
```
