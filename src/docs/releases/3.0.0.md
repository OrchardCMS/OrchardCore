...
We've added a new **CollapseContainedItems** setting to **BagPartSettings** and **FlowPartSettings**, allowing users to control whether contained items are collapsed or expanded by default. To ensure consistency, both **BagPart** and **FlowPart** will now expand contained items when the page loads by default.  

!!! note  
    To preserve existing behavior, this migration sets **CollapseContainedItems** to **true** for **BagPart** in existing tenants.  
