...
Previously, the ReCaptcha module included a shape type called "FormReCaptcha," which allowed you to use the driver to display the ReCaptcha challenge on any form. However, due to the dynamic nature of ReCaptcha and the threshold settings that control when the challenge is displayed, it is crucial to inject ReCaptcha using the `ShapeTableProvider`.

If you're currently using `FormReCaptcha` in your project, you should transition to injecting it through an implementation of `IShapeTableProvider`. To simplify this process, we’ve introduced a new base class, `ReCaptchaShapeTableProvider`, which automatically injects the ReCaptcha challenge for specific shape types. For example, we inject it on the login page in Orchard Core as follows:

```csharp
services.AddShapeTableProvider<ReCaptchaUserShapeTableProvider>();
```

The `ReCaptchaUserShapeTableProvider` class inherits from the new `ReCaptchaShapeTableProvider`, which you can also use directly:
...
Alternatively, if you must continue rendering ReCaptcha using the deprecated `FormReCaptcha`, you can manually add the `FormReCaptcha.cshtml` view to your project with the following code:
