...
JavaScript global methods received the new `GlobalMethod.AsyncMethod` property, and all built-in methods got a matching implementation. In most cases, this is non-breaking, for example, a recipe that contains

```json
"ContentItemId": "[js: variables('blogContentItemId')]"
```

will continue to work, because the expression returns a JavaScript `Promise` object, which is automatically awaited. However, if you want to do any operation with the result before returning it, that has to be awaited inside the JavaScript expression. For example, in the Blog recipe

```json
"LinkUrl": "[js: variables('adminUrlPrefix') + '/Contents/ContentItems/' +  variables('blogContentItemId') + '/Display']",
```

had to be replaced with

```json
"LinkUrl": "[js: (async function() { return `${await variables('adminUrlPrefix')}/Contents/ContentItems/${await variables('blogContentItemId')}/Display`; })()]",
```

to avoid a situation where the value would be evaluated as `/Admin/[object Promise]/Contents/ContentItems/[object Promise]/Display`. This is because the original JavaScript expression tried to add a `Promise` object and a `string` together, which results in a `string`. Note how the JavaScript expression is wrapped in an [async function](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/async_function) because you can't use `await` in an expression otherwise. Alternatively, you can use [`.then()` chaining](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Using_promises#chaining) if you prefer to avoid the wrapper function.
