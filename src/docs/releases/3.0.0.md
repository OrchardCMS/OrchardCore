...
#### ImageSharp replaced by NetVips

On-demand image processing is no longer backed by `SixLabors.ImageSharp` and `SixLabors.ImageSharp.Web`. Image resizing is now performed by a built-in middleware using [NetVips](https://github.com/kleisauke/net-vips), a managed binding over the native [libvips](https://www.libvips.org/) library. This change was motivated by the move of ImageSharp to a commercial/split license, and brings significantly faster resizing through shrink-on-load decoding.
