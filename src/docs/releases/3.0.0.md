...
**Implementation:**
- A new middleware (`CommitSessionMiddleware`) automatically commits the `IDocumentStore` or `ISession` before the response starts
...
- The BeforeDispose callback checks if the commit was already performed via the middleware and only commits if needed
