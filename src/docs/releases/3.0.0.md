...
In the previous implementation, the ReCaptcha module supported two modes: `AlwaysShow` and `PreventRobots`. To simplify the module and streamline the integration, the concept of `PreventRobots` and its associated mode have been removed. Going forward, only the `AlwaysShow` mode is supported.

As part of this update, the following components have been removed:

- `IDetectRobots`
- `IPAddressRobotDetector`
- `ReCaptchaMode`

Additionally, the `ReCaptchaService` class has been sealed to prevent inheritance. The following methods were also removed:
