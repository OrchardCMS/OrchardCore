...
A new shape, `UserDisplayName` has been introduced to render a user's display name in a consistent and cache-friendly way. This is frequently used in admin content lists.

While this is not a breaking change, if you override the affected shapes (see the changes in the [relevant pull request](https://github.com/OrchardCMS/OrchardCore/pull/18329/files)), we recommend you make use of `UserDisplayName` too.
