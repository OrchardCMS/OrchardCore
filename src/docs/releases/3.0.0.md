...
Previously, the `ILocalizationService` interface has a `static` default method called `GetAllCulturesAsync()`, which returned a list of all cultures available in the system with their aliases.

This method has been replaced in the current version, and becomes an intance method to allow developers customize the implementations instead of relying on the default onces.
