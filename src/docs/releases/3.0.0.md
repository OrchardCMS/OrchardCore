...
The `IExternalLoginEventHandler` interface in the External Authentication feature actually contained methods that produced various values, rather than events. To fix this inconsistency, we renamed it to `IExternalLoginUserHandler`, but the behavior remained the same. If you have an existing implementation, just change it to implement `IExternalLoginUserHandler` (no other changes necessary).
