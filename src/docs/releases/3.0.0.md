...
!!! warning
    Because the enabled-feature state is persisted by feature id, renaming a feature id disables it on existing sites: after upgrading, a site that had `OrchardCore.Media.Azure.ImageSharpImageCache` (or `OrchardCore.Media.AmazonS3.ImageSharpImageCache`) enabled will no longer have the image cache feature active, because the stored id no longer matches any feature. Re-enable the renamed feature (`OrchardCore.Media.Azure.ImageCache` or `OrchardCore.Media.AmazonS3.ImageCache`) from the *Features* admin screen, a migration, or your setup recipe after upgrading.
