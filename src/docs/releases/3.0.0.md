...
in the `DefaultContentManager` class returned content items in the same order as the provided list of IDs.

Starting with this release, that behavior has changed: **the returned content items are no longer guaranteed to follow the input order**.

If you need to preserve the original ordering, use the new extension method:

```csharp
Task<IOrderedEnumerable<ContentItem>> GetOrderedContentItemsAsync(
...
This method is specifically designed to guarantee that the returned items match the order of the given IDs.
