...
- **`Standard`**: A typical role that allows granular permission assignment.
- **`System`**: Non-removable system roles such as 'Anonymous' and 'Authenticated.'
- **`Owner`**: A special role type that automatically grants all permissions without needing to specify them. This type is designed for roles like `Administrator` or `SiteOwner`.

!!! warning
    Prefer to set the `Owner` type on roles instead of using the `SiteOwner` permission. The latter will be removed in a future version. Don't authorize against the `SiteOwner` permission in code.
!!! note
    When a user with a role marked as `Owner` logs in, role-specific claims are excluded from the claims principal, as full access is already granted.
