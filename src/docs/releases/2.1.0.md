...
!!! warning
    If you're using a custom `Login.cshtml` view or `Login` template, make sure to update the external login form action. The `ExternalLogin` action has been moved from the `Account` controller to the `ExternalAuthentications` controller as of this update.
