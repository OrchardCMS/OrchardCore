...
A `PollingTime` & `MinimumIdleTime` have been added to the `BackgroundServiceOptions`, now we have the ability to configure them from with the `appsettings.json`.
