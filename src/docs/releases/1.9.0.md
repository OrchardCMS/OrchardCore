...
In the past, we utilized the injection of `ISmtpService` for sending Email messages. However, in this release, it is now necessary to inject `IEmailService` instead.

Additionally, the `SMTP` provider is no longer enabled by default. If you want to use SMTP email provider, you must enable the provider by visiting the email settings `Configuration` > `Settings` > `Email` and see the `SMTP` tab.

### *New* Email Azure Module
