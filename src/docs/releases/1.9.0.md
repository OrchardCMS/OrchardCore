...
**Migration Process**
    - If the email settings have been configured in the site settings
        1. Copy the settings before upgrade.
        2. Apply the upgrade process.
        3. Re-configure settings.
    - If the email settings have been configured in the `appsettings.json`
        1. You can still use the old `OrchardCore_Email` section, but we encourage to use `OrchardCore_Email_Smtp` section instead.
