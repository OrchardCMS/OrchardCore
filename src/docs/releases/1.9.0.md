...
The recent addition in the [Pull Request](https://github.com/OrchardCMS/OrchardCore/pull/15875) introduces the `IShellContextReleaseService`, enabling you to initiate a shell release at the request's conclusion via the `RequestRelease()` method. This action is accessible from any service within the application. However, there's no assurance of immediate fulfillment since another service may utilize the same service and defer the release request later. Should you need to postpone the shell restart request until the current request completes execution, you can utilize the `SuspendReleaseRequest()` method to suspend the tenant release request.

#### Reloading Tenants Display Drivers

When implementing a site settings display driver, you have the option to reload the shell (restart the tenant). If you choose to do so, manually releasing the shell context using `await _shellHost.ReleaseShellContextAsync(_shellSettings)` is unnecessary. Instead, you should use the new `IShellContextReleaseService.RequestRelease()`. This ensures that the shell stays intact until all settings are validated. For instance, in `ReverseProxySettingsDisplayDriver`, the code was modified from this.
