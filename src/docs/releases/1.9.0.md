...
Additionally, `SMTP` provider is no longer enabled by default. If you want to use SMTP email provider, you must enable the provider by visiting the email settings `Configuration` > `Settings` > `Email` and see the `SMTP` tab.
