...
`TheAdmin` now includes a quick search command palette. Press <kbd>Ctrl</kbd>+<kbd>K</kbd> (<kbd>Cmd</kbd>+<kbd>K</kbd> on macOS) or click the magnifier icon in the admin navbar to open it, type to filter the admin menu items you have access to (matching is case- and accent-insensitive and includes the parent menu names, shown as a breadcrumb), then use the arrow keys and <kbd>Enter</kbd> to navigate to the selected page.

The palette is enabled by default and can be turned off from **Configuration → Settings → Admin** (**Enable quick search**) or with the new `DisplayQuickSearch` property of `AdminSettings` in a `settings` recipe step. See the [Admin documentation](../reference/modules/Admin/README.md#quick-search) for details.
