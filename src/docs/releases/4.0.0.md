...
The content management screens are the first to use it: the content items list, the content item creation form and the content item edition form render the `Contents`, `ContentsCreate` and `ContentsEdit` trails, whose names and data keys are exposed by `OrchardCore.Contents.ContentsBreadcrumbs`. A trail is named after the screen that renders it, and reuses the name that screen already publishes for something else of its own, so that the content items list is `Contents` both as an admin list and as a breadcrumb. The other admin screens keep their title for now.
