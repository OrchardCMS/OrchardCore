...
A trail is identified by a name and is built by every `IBreadcrumbProvider` registered on the tenant, which is what makes it extensible: a module adds a node to, or removes a node from, a trail described by another module without either of them knowing about the other. A view renders one with the new `<breadcrumb>` tag helper:
...
A screen whose trail is static can skip the provider and declare its nodes inline as `breadcrumb-item` children of the tag helper; the providers still run over the inline trail, so it stays extensible. The `data` attribute carries the contextual data of the page, which every provider of the trail reads back through `BreadcrumbBuilder.Data`. Nodes are ordered with the placement syntax, can require permissions, and are rendered as plain text rather than dropped when the user is not authorized to reach them, so that the trail stays complete. Both the `Breadcrumb` and the `BreadcrumbItem` shapes carry alternates built from the name of the trail, the id of the node and the display type of the trail, so a single screen can be templated on its own, and a theme can give the admin and the front end a presentation of their own: the tag helper sets the display type to `DetailAdmin` on a request to the admin and to `Detail` everywhere else. The heading follows the same split: on the admin the current node's text is rendered below the trail as an `<h1>`, the title of the screen, and no title elsewhere, where the page has a heading of its own.

Every built-in admin screen now renders a breadcrumb in place of its title, from the content items list to the search index, deployment plan, admin menu, rate limit and OpenID screens. Each nested editor leads back through its parent — a menu item through its menu, a deployment step through its plan — so the trail always mirrors how the screen was reached. A trail is named after the screen that renders it, and reuses the name that screen already publishes for something else of its own, so that the content items list is `Contents` both as an admin list and as a breadcrumb. The names and data keys of a module's trails are exposed on its `{Module}Constants` class (for example `OrchardCore.Contents.ContentsConstants`, with the `Contents`, `ContentsCreate` and `ContentsEdit` trails).
...
The `OrchardCore.AdminDashboard` feature shows what injection looks like: when it is enabled, it adds the dashboard at the beginning of **every** admin trail, so `Manage Content › Edit Article` becomes `Dashboard › Manage Content › Edit Article`. Disabling the feature removes the node again, and the content management screens never learn that the dashboard exists.

The trail is on by default and can be turned off for the whole admin with the **Show breadcrumb** setting under _Configuration → Settings → Admin_ (`AdminSettings.ShowBreadcrumb`); when it is off, each screen renders its page title on its own, without the trail above it.
