...
Module authors that write temporary files should inject `ITempDirectoryProvider` (`GetRootDirectory()`, `GetOrCreateSubdirectory()`, `GetTempFileName()`, `CreateTempSubdirectory()`) instead of calling `Path.GetTempPath()`, so their temporary files honor this configuration and stay scoped per tenant. The default `DefaultTempDirectoryProvider` is registered with `TryAddSingleton`, so it can be replaced with a custom filesystem-backed implementation. See the [Temporary File Storage documentation](../reference/core/temporary-file-storage.md) for configuration, mounting a file share, and consuming the interface.
