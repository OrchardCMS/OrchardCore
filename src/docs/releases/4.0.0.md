...
The 401 and 403 responses produced by the shared `Api` authentication scheme now carry an RFC 9457 Problem Details body (`application/problem+json`) alongside the `WWW-Authenticate` header, so JavaScript API clients get a parseable payload without having to parse the header. The header itself is still the responsibility of the authentication handler that issued the challenge (e.g. the OpenID Token Validation feature, which emits the RFC 6750 header), and it is left untouched. The body is written through `IProblemDetailsService`, so an application registering its own Problem Details customizations sees them applied to these responses too.
