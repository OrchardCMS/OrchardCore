...
Admin screens can now render a breadcrumb trail instead of a bare title. The `Breadcrumb` shape, provided by the always-enabled `OrchardCore.Navigation` module, renders the trail of pages leading to the page being displayed, e.g. _Manage Content › Edit Site Page_, with the last node carrying the `<h1>` of the page. It replaces the title, so it works in the top bar and in the content area alike, and it registers the text of its current node as a segment of the `<title>` of the page.
