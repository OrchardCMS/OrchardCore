...
A new `ILogoutFormEvent` interface (with the `LogoutFormEventBase` convenience base class) lets modules hook into the logout process, complementing the existing `ILoginFormEvent`. Implementations are invoked when a user signs out through the account logout endpoint: `LoggingOutAsync(IUser user)` runs before the user is signed out (while the principal is still available) and `LoggedOutAsync(IUser user)` runs afterward. Register a handler like any other event service:
