...
`OrchardCore.Admin` now includes a quick navigation palette, with its default view, navbar driver, JavaScript, and CSS provided by the module rather than an admin theme. Admin themes using the `DetailAdmin` navbar inherit the palette and can override its styles without adding navigation-specific files. Press <kbd>Ctrl</kbd>+<kbd>K</kbd> (<kbd>Cmd</kbd>+<kbd>K</kbd> on macOS) or click the magnifier icon in the admin navbar to open it. Type to filter the admin menu items you can access (matching is case- and accent-insensitive and includes parent menu names, shown as a breadcrumb), then use the arrow keys and <kbd>Enter</kbd> to navigate to the selected page. The palette loads a fresh localized, permission-filtered JSON index on every open. Caching is source-owned; the controller always executes the sources and renders their results. Opt-in features can add sources through `QuickNavigationSource` and update the tenant's in-memory `IQuickNavigationIndex` from background tasks or event handlers; only admin menu items are provided by default.

The palette is enabled by default and can be turned off from **Configuration → Settings → Admin** (**Enable quick navigation**) or with the new `DisplayQuickNavigation` property of `AdminSettings` in a `settings` recipe step. See the [Admin documentation](../reference/modules/Admin/README.md#quick-navigation) for details.

The opt-in **Content Quick Navigation** feature (`OrchardCore.Contents.QuickNavigation`) adds edit links for the 50 most recently updated content items, using their display text and respecting edit permissions. The count is configurable in Admin settings through `ContentQuickNavigationSettings.MaxItems`. See the [Contents documentation](../reference/modules/Contents/README.md#content-quick-navigation).
