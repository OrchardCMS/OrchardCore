...
- First level items holding children are now rendered as groups: a small uppercase caption followed by the items they hold, which are visible without a click. Collapsing a group is still possible, and the groups a user collapses are remembered in the admin preferences.
- Items nested deeper stay collapsed until they are expanded, as before.
...
- The `ul` of a group carries a `nav-group-items` class, its `id` is now built from the stable hash of the item (`m{hash}`) instead of the shape's runtime hash code, and the `li` of a first level item carries a `nav-group` class.
...
The top bar and the menu of **TheAdmin** theme now use a silver surface tinted with the Orchard Core green of the logo, `#41B670`, instead of the neutral grays derived from the Bootstrap subtle backgrounds, in both the light and the dark theme. The selection indicator, the hover overlays, and the background of the current menu item are all derived from that accent, which replaces `var(--bs-primary)` as the value of `--oc-selected-indicator-color`.
...
```css
:root {
    --oc-admin-accent-rgb: 65, 182, 112;
    --oc-admin-accent-strong-color: #2b8d57;
    --oc-admin-chrome-bg-color: #f7faf8;
    --oc-admin-menu-bg-color: #eef3f0;
    --oc-admin-menu-border-color: #dce5e0;
}
```

`--oc-admin-menu-separator-color`, `--oc-admin-menu-item-hover-bg-color`, and `--oc-admin-menu-item-active-bg-color` are now tinted with `--oc-admin-accent-rgb` instead of the Bootstrap emphasis color, and `--oc-admin-menu-group-font-color`, `--oc-admin-menu-border-color`, `--oc-admin-chrome-bg-color`, `--oc-admin-accent-rgb`, `--oc-admin-accent-color`, and `--oc-admin-accent-strong-color` are new. `--oc-admin-accent-strong-color` is the shade used where the accent has to stay readable against a surface: darkened in the light theme, lightened in the dark one.
