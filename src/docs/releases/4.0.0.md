...
The Media Library page now returns `404 Not Found` instead of `403 Forbidden` when the current user may not use it, matching the media middleware's behavior of concealing what a user cannot see. It is also gated on the same check the gallery itself performs, rather than on `ManageMediaContent` alone, so a user who can list nothing is turned away instead of landing on an empty library that fails every request. The **Media Library** admin menu entry is hidden in that case as well.

### Secure Media Module

With **Secure Media** enabled, a per-folder `View media content in folder '…'` permission now grants access to the media root only in order to navigate into that folder. It no longer grants access to the files stored directly in the root, which belong to no folder and are covered by `ViewRootMediaContent` ("View media content in the root folder") alone. Grant that permission to any role expected to see media stored at the top level.

The **Secure Media** feature no longer grants `ViewMediaContent` ("View media content in all folders") to the **Anonymous** role by default. A feature whose purpose is restricting access to media should not open every folder to the public when it is enabled. This also removes a subtle trap: Anonymous role claims are evaluated for authenticated users too, so that grant silently overrode every folder-scoped restriction placed on signed-in roles.

Default permissions are applied when a role is created, so existing roles keep the claim they already hold and existing sites are unaffected. On a site where Secure Media is enabled **for the first time**, media requests from unauthenticated visitors now return `404 Not Found`. To serve media publicly again, grant Anonymous either `ViewMediaContent` for all folders, or the per-folder `View media content in folder '…'` permissions for the folders that should stay public.
...
### Media Library Access for Own-Folder Roles

With **Secure Media** enabled, a role whose only media grants were `ManageOwnMediaContent` and `ViewOwnMediaContent` could not open the Media Library at all. Every media endpoint first checks access to the media root, and reaching the root was satisfied only by the root permission, the global `ViewMediaContent`, or a per-folder permission — none of which the own-media permissions imply. There was therefore no permission combination expressing "let this role use the Media Library, but only inside its own folder".

Viewing the media root now also accepts `ViewOwnMediaContent` and `ViewOthersMediaContent`. Reaching the root reveals nothing by itself: the folder listing still filters each folder by permission, and the root's own files require `ViewRootMediaContent` as described under Breaking Changes. Such a role now sees the users folder and its own folder within it, and can create subfolders there.
