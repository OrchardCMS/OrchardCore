...
When a tenant maps a dynamic controller route (as `OrchardCore.Autoroute`, `OrchardCore.HomeRoute` and `OrchardCore.Sitemaps` do), the shared controller endpoint data source also emits a second, non-routable placeholder endpoint for every action, copying the action metadata — including its `[EndpointName]`. For attribute-routed actions this collided with the routable endpoint's name, and the framework's default endpoint-name address scheme throws as soon as two link-generating endpoints share a name.

`OrchardCore.Mvc` now registers a tenant-aware `ShellEndpointNameAddressScheme` (alongside the existing `ShellRouteValuesAddressScheme`) that tolerates duplicate endpoint names: it returns the endpoints registered under the requested name and lets the link generator select the one that can produce a URL. This keeps all tenant link generation robust regardless of how the duplicate-named endpoints were produced.
