...
##### Deprecated: `api/media/GetAllMediaItems`

This endpoint walks the entire media library and returns every item in a single response, with no paging, depth limit or item cap. On a remote store that is one round-trip per directory, so a single request can enumerate the whole account.

Nothing in Orchard Core calls it any more — the Vue 3 gallery loads one directory at a time through `GetDirectoryContent` and pages the folder tree as you scroll — and paging it would contradict what its name promises. It is therefore deprecated and will be removed in a future release. Responses now carry a `Deprecation` header pointing at `GetDirectoryContent`, which is the replacement for walking the library a directory at a time.
