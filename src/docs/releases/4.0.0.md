...
Liquid execution is now isolated to `LiquidPart`, the new `LiquidField`, and explicit Liquid template APIs. `HtmlBodyPart`, `HtmlField`, `MarkdownBodyPart`, and `MarkdownField` no longer validate or execute embedded Liquid. Existing Liquid syntax remains stored unchanged but is rendered as ordinary HTML or Markdown source. Migrate definitions that previously enabled `RenderLiquid` manually to `LiquidPart` or `LiquidField`; tenant startup logs a warning for each affected content type, part, and field.

The new security-critical `ManageLiquidTemplates` permission is required in addition to existing permissions when editing Liquid parts and fields, Templates, Admin Templates, or template previews. It is granted to Administrator and Editor by default, and the migration preserves existing Editor access.

HTML and Markdown source is now persisted exactly as authored. When `SanitizeHtml` is enabled, sanitization occurs only on final rendered HTML: HTML source is processed through shortcodes and then sanitized; Markdown source is converted to HTML, processed through shortcodes, and then sanitized. This applies consistently to Detail, Summary, Preview, `BodyAspect`, GraphQL rendered HTML, and the Markdown Razor helper. `SanitizeHtml = false` remains the trusted-output opt-out.

`HtmlMenuItemPart` follows the same source-preservation model. Public menu rendering sanitizes HTML on a cloned content item and rejects unsafe URL schemes without mutating persisted or cached content.
