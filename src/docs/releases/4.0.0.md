...
`MediaOptions.TusTempPath` no longer defaults to `{TempPath}/TusUploads`; when it is left unset, the TUS disk store now follows the shared temporary file store location. An explicitly configured `TusTempPath` still takes precedence, so existing configurations are unaffected.

Because these consumers require random-access local files (for example `ZipFile.ExtractToDirectory` and `PhysicalFileProvider`), point `TempPath` at a mounted file share rather than object storage. Resumable (TUS) partial uploads can still target Azure Blob or AWS S3 object storage through the `ITusTempStore` implementations in the `OrchardCore.Media.Azure` and `OrchardCore.Media.AmazonS3` features.

Module authors that write temporary files should inject `ITempWorkspace` (`GetRootDirectory()`, `GetOrCreateSubdirectory()`, `GetTempFileName()`, `CreateTempSubdirectory()`) instead of calling `Path.GetTempPath()`, so their temporary files honor this configuration and stay scoped per tenant. The default `DefaultTempWorkspace` is registered with `TryAddSingleton`, so it can be replaced with a custom filesystem-backed implementation. See the [Temporary File Storage documentation](../reference/core/temporary-file-storage.md) for configuration, mounting a file share, and consuming the interface.
