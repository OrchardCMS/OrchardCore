...
### Default JavaScript Execution Timeout

Scripts evaluated through `IScriptingManager` now run under a 30 second execution timeout, where previously no execution constraint was configured and a script such as `while (true) {}` ran until the process was recycled. A script is evaluated on the thread of its caller — a layer rule runs on the thread of the request it applies to — so a runaway script no longer costs a thread permanently. A script that exceeds the timeout fails with a `TimeoutException`.

Sites running legitimately long scripts can change the value, or turn the timeout off, from the `OrchardCore_Scripting_JavaScript` configuration section:

```json
{
  "OrchardCore_Scripting_JavaScript": {
    "TimeoutInterval": "00:01:00"
  }
}
```

See the [Scripting documentation](../reference/modules/Scripting/README.md#default-execution-timeout) for the settings and their code equivalents.
