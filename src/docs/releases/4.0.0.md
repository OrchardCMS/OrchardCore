...
The built-in deployment steps now use `LocalizedString.Create()` to set their `Category` in their parameterless constructor, and the constructors that took an `IStringLocalizer<TStep>` only to set the category were removed. The deployment plan page now translates the category by its name, with the step type as the context, which is the same context that `IStringLocalizer<TStep>` uses. Existing translations continue to work, and custom steps that still set the category with an `IStringLocalizer<TStep>` are not affected. See [Extending deployment](../reference/modules/Deployment/README.md#extending-deployment).
