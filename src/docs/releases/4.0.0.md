...
- Each list has a name, e.g. `Queries` or `Features`, published as a constant next to its default columns, e.g. `QueriesAdminList.Name`. It selects both the columns a provider configures and the `AdminList__{Name}` alternate.
