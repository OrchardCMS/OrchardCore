...
`TheAdmin` now includes a quick search command palette. Press <kbd>Ctrl</kbd>+<kbd>K</kbd> (<kbd>Cmd</kbd>+<kbd>K</kbd> on macOS) or click the magnifier icon in the admin navbar to open it. Type to filter the admin menu items you can access (matching is case- and accent-insensitive and includes parent menu names, shown as a breadcrumb), then use the arrow keys and <kbd>Enter</kbd> to navigate to the selected page.
