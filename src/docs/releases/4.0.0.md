...
`TheAdmin` now includes a quick navigation palette. Press <kbd>Ctrl</kbd>+<kbd>K</kbd> (<kbd>Cmd</kbd>+<kbd>K</kbd> on macOS) or click the magnifier icon in the admin navbar to open it. Type to filter the admin menu items you can access (matching is case- and accent-insensitive and includes parent menu names, shown as a breadcrumb), then use the arrow keys and <kbd>Enter</kbd> to navigate to the selected page. The palette loads a fresh localized, permission-filtered JSON index on every open. Caching is source-owned; the controller always executes the sources and renders their results. Opt-in features can add sources through `QuickNavigationSource` and update the tenant's in-memory `IQuickNavigationIndex` from background tasks or event handlers; only admin menu items are provided by default.
