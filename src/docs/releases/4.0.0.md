...
The configuration sections are no longer prefixed with `OrchardCore_`. They are grouped by service instead, with a nested section for each provider of the service, so that all the settings of a service are located together. For instance, the `OrchardCore:OrchardCore_Email_Smtp` section is now `OrchardCore:Email:Smtp`, next to the `OrchardCore:Email:Azure` section, and the `OrchardCore:OrchardCore_Elasticsearch` section is now `OrchardCore:Search:Elasticsearch`, next to the `OrchardCore:Search:AzureAISearch` section:
...
The shorter names also keep the environment variables and Azure Key Vault secret names short, which matters for hosts limiting their length. For instance, the `OrchardCore__OrchardCore_Media_Azure__ConnectionString` environment variable becomes `OrchardCore__Media__Azure__ConnectionString`.

The previous names are deprecated but are still supported, in addition to the new names, so existing configurations keep working. When a setting is defined using both names, the value defined with the new name is used, and an array defined with the new name replaces the array defined with the previous name. **It is recommended to rename your configuration sections, in your `appsettings.json` files, environment variables, and other configuration sources, as the previous names will be removed in a future major version.**

The following sections are renamed, all of them being nested in the `OrchardCore` section, e.g. `OrchardCore:Media:Azure`:

| Previous section name | New section name |
| --- | --- |
...
!!! note
    The Azure and Amazon S3 media image cache sections are renamed from their names in the 3.0 release, `OrchardCore_Media_Azure_ImageSharp_Cache` and `OrchardCore_Media_AmazonS3_ImageSharp_Cache`, which are still supported. The `OrchardCore:Antivirus:ClamAV` section and the `OrchardCore:Liquid:PermittedSiteProperties` setting are introduced in this release and don't have a previous name.

The `appsettings.json` file of the `OrchardCore.Cms.Web` project now uses the new names. Module authors can use the `ConfigurationSectionExtensions.GetSectionCompat(key, params legacyKeys)` extension method to rename their own configuration sections the same way, as it gets a section merged with its legacy sections.

### IntelliSense and Validation of the `appsettings.json` Files

The Orchard Core packages that read a configuration section now include a `ConfigurationSchema.json` file describing their sections, declared as a JSON schema segment that flows to the referencing projects, for instance through the `OrchardCore.Application.Cms.Targets` package. Visual Studio uses these schemas to provide IntelliSense, descriptions and validation when editing the `appsettings.json` and `App_Data/appsettings.json` files of a project: the section and setting names are suggested, as well as the known values of settings like the database providers, the setup recipes, the time zones or the cultures, and the deprecated section names are flagged. See the [Configuration documentation](../reference/modules/Configuration/README.md#intellisense-and-validation-of-the-appsettingsjson-files) for more details.

### External Login Secrets from Configuration

The Facebook `AppSecret`, the Google `ClientSecret`, the Microsoft Account `AppSecret`, and the X `ConsumerSecret` and `AccessTokenSecret` settings can now be provided through the configuration, e.g. from environment variables. These secrets were previously expected to be encrypted like when they are saved from the admin settings, so the values provided through the configuration could not be decrypted and were ignored.

The X API client also no longer fails after its first request, as it doesn't replace the encrypted secrets of the shared settings by their decrypted values anymore.
...
- A new `UseHierarchicalNamespace` setting on the Azure Media Storage configuration (`OrchardCore:Media:Azure`) lets you force Gen1 or Gen2 behavior when account-level detection is unavailable, such as when connecting with a container-scoped SAS token.
