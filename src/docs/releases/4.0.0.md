...
`Permission.Description` is now a `LocalizedString` instead of a `string`, so that permission descriptions can be translated with PO files. The constructors that take a `string` description are still available, and new constructors take a `LocalizedString` description. Code that reads `Description` as a `string` still compiles, because `LocalizedString` converts to `string`. Code that assigns a `string` to `Description`, or calls `string` methods on it directly, must use `LocalizedString.Create(...)` or `Description.Value` instead. Modules compiled against a previous version must be recompiled.
...
### `LocalizedString.Create` and `LocalizedHtmlString.Create`

`OrchardCore.Abstractions` now adds a static `Create(string value, params object[] arguments)` method to `LocalizedString` and `LocalizedHtmlString` through C# 14 extension members. The method creates an instance from a value, and also uses the value as the name, so `LocalizedString.Create("Hello")` replaces `new LocalizedString("Hello", "Hello")`. This works because Orchard Core uses the English text both as the default value and as the name of a localized string. Like `S["Hello {0}", name]`, the method also accepts arguments, for example `LocalizedString.Create("Hello {0}", name)`. The extensions are in the `Microsoft.Extensions.Localization` and `Microsoft.AspNetCore.Mvc.Localization` namespaces, the same namespaces as the types. See [Creating a localized string without a localizer](../reference/modules/Localize/README.md#creating-a-localized-string-without-a-localizer).

### Localized Permission Descriptions

The built-in permissions now create their descriptions with `LocalizedString.Create(description, typeof(DeclaringType))`. The new `LocalizedString.Create(string value, Type resourceSource)` overload keeps the full name of the type in `LocalizedString.SearchedLocation`, and the new `IStringLocalizerFactory.Localize(LocalizedString)` extension translates the string later with that context, the same context that `IStringLocalizer<T>` uses. The roles editor, and the permission pickers of admin menu nodes and menu items, now show the PO file translation of these descriptions. Because these descriptions are translated with PO files, they are no longer listed in Data Localization, and translations of them that were entered in Data Localization are no longer used. Descriptions that are plain strings, including the permissions built from templates such as "Edit Article", and permission categories continue to be translated with Data Localization. See [Declaring permissions](../reference/modules/Roles/README.md#declaring-permissions).

### Deployment Step Categories and Titles

The built-in deployment steps now use `LocalizedString.Create()` to set their `Category` and `Title` in their parameterless constructor, and the constructors that took an `IStringLocalizer<TStep>` only to set them were removed. The deployment plan page and the step screens now translate the category and the title by their name, with the step type as the context, which is the same context that `IStringLocalizer<TStep>` uses. The new `DeploymentStep.GetLocalizedCategory()` and `DeploymentStep.GetLocalizedTitle()` extensions do this translation. Existing translations continue to work, and custom steps that still set the category or the title with an `IStringLocalizer<TStep>` are not affected. See [Extending deployment](../reference/modules/Deployment/README.md#extending-deployment).
