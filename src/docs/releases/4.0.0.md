...
- Every column of a list comes from an `IAdminListColumnProvider` registered with `services.AddAdminListColumnProvider<TProvider>()`, including the ones of the module owning the list, e.g. `QueriesAdminListColumnProvider`. Other modules add, remove or change columns the same way, and themes can override a cell with `AdminListCell-{Column}.cshtml` or `AdminListCell-{ListName}-{Column}.cshtml`. `AdminListColumns.Select()` and `AdminListColumns.Actions()` declare the selection and the actions columns most lists share. A column name is unique in its list: `context.Columns` is keyed by it, so adding a second column with the same name throws.
