...
### Local installation ports

`pomi install` selects an available random HTTPS localhost port by default and
checks explicitly requested `--urls` ports before installation. It saves the
selected addresses in the generated application and launch settings so restarts
reuse them. See [local installation](../guides/remote-management/README.md).
