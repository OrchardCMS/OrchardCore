...
The standalone app fetches its UI labels anonymously before login through a new, generic **JavaScript Localization API** (`GET api/localization/js/{group}`, part of the independently toggleable `OrchardCore.Localization.Js` feature in the `OrchardCore.Localization` module), which merges any module's registered `IJSLocalizer` translations for the requested group(s) — the same mechanism a Razor view uses via `Orchard.GetJSLocalizations(...)`. The `MediaApiStandalone` recipes enable this feature automatically. See [JavaScript Localization](../reference/modules/Localize/javascript-localization.md) for the full contract.
