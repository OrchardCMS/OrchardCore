...
The `data` attribute carries the contextual data of the page, which every provider of the trail reads back through `BreadcrumbBuilder.Data`. Nodes are ordered with the placement syntax, can require permissions, and are rendered as plain text rather than dropped when the user is not authorized to reach them, so that the trail stays complete. Both the `Breadcrumb` and the `BreadcrumbItem` shapes carry alternates built from the name of the trail and the id of the node, so a single screen can be templated on its own.
