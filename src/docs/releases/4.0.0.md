...
**Let users choose the layout of a list** is a new admin setting. With it on, every list shows a selector with the layouts the site can render, a click reloads the page with `?layout=Grid`, and the choice is kept in a cookie per list, so one user reads the content items as a table while another reads them as a grid, without changing anything for the site. The pager and the page size selector carry the layout, and a layout the site cannot render is ignored wherever it comes from. `IAdminListService.GetAvailableLayoutsAsync()` returns the layouts of the site, and a layout ships the button offering it as `AdminListLayoutSelectorItem-{Layout}.cshtml`.
