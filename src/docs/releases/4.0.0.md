...
- **`OrchardCore.Media.SignalR`** — real-time media updates broadcast to all connected clients, keeping the gallery in sync across tabs, users, and instances. It builds on the reusable **`OrchardCore.SignalR`** module, whose **`.Azure`** and **`.Redis`** features provide a shared backplane for every hub in the tenant.
