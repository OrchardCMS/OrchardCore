...
Admin lists can now be rendered with different layouts. The `OrchardCore.Admin` module provides a reusable `AdminList` shape with three built-in layouts: `List` (the existing look), `Table`, which renders the items as a table with one column per `AdminListColumn`, and `Grid`, which renders the same columns with flex rows instead of a `<table>`. Both column layouts stack the cells on small screens instead of scrolling horizontally. A column defines its `Width` (any CSS width, or `auto` to hug the content), `Alignment` and `NoWrap`. The row actions are rendered by the new `AdminListActions` shape in the layout selected under **List actions layout**: `Buttons` (the existing look) or `Menu` (a single compact dropdown opened by an ellipsis button). The default layout is selected in **Configuration → Settings → Admin** under **List layout**. The content items, users and indexes lists use it; the other admin lists will follow. Lists without an options editor can render the new `AdminListToolbar` shape (item count, select-all and bulk actions) above the items.
