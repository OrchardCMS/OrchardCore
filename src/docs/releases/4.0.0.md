...
After providers finish, the parent helper orders ancestors stably with `FlatPositionComparer`, preserving their current list order for equal positions, then unconditionally appends the explicit title node as final/current and never linked. This remains true if providers clear every ancestor or use `Position = "end"`; providers cannot remove or replace the current title. Ancestors support MVC action routes, `route-*` parameters or a `route-values` dictionary (`IDictionary<string, string>`), direct URLs, permission names, authorization resources, and `link-enabled` (default `true`). A supplied `action` takes precedence over `url`. Unauthorized ancestors and those with `link-enabled="false"` remain visible as plain text. Child text is HTML-decoded, the normalized title and provider text are plain strings, and output is safely encoded.
...
The `OrchardCore.AdminDashboard` feature registers `OrchardCore.AdminDashboard.Services.DashboardBreadcrumbProvider`. For visible trails, it checks `AdminAttribute.IsApplied(context.ViewContext.HttpContext)` and inserts a localized Dashboard ancestor at index `0` with position `start` and ID `Dashboard`, unless `context.Name == "Dashboard"` or an ancestor already has that ID. It does not compare localized title text. Its URL is `"~/" + AdminOptions.AdminUrlPrefix`, and its `Permissions` list uses the static `OrchardCore.AdminDashboard.Permissions.AccessAdminDashboard` permission object. It can add Dashboard when no other ancestors exist, but never supplies the current title or changes front-end trails. Hidden admin trails skip this provider entirely.
