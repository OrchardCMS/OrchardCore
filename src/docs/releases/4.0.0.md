...
The built-in permissions now create their descriptions with `LocalizedString.Create(description, typeof(DeclaringType))`. The new `LocalizedString.Create(string value, Type resourceSource)` overload keeps the full name of the type in `LocalizedString.SearchedLocation`, and the new `IStringLocalizerFactory.Localize(LocalizedString)` extension translates the string later with that context, the same context that `IStringLocalizer<T>` uses. The roles editor, and the permission pickers of admin menu nodes and menu items, now show the PO file translation of a permission description when there is one. Otherwise, the roles editor continues to use the Data Localization translation. See [Declaring permissions](../reference/modules/Roles/README.md#declaring-permissions).
