...
`OrchardCore.Localization.Abstractions` now adds a static `Create(string name)` method to `LocalizedString` and `LocalizedHtmlString` through C# 14 extension members. The method creates an instance that uses the same text for its name and its value, so `LocalizedString.Create("Hello")` replaces `new LocalizedString("Hello", "Hello")`. The extensions are in the `Microsoft.Extensions.Localization` and `Microsoft.AspNetCore.Mvc.Localization` namespaces, the same namespaces as the types. See [Creating a localized string without a localizer](../reference/modules/Localize/README.md#creating-a-localized-string-without-a-localizer).
