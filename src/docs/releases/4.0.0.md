...
Disabling `RequireEndSessionConfirmation` in the OpenID server settings now signs users out without a confirmation prompt, as intended. Previously the prompt was always shown, because the `id_token_hint` was not available to the logout action after OpenIddict's end session request caching redirect. The local authentication cookie is now also removed when the prompt is skipped.

The `id_token_hint` must be a valid identity token issued to the requesting client (resolved from `client_id` or `post_logout_redirect_uri`) for the currently authenticated user. Otherwise, the confirmation prompt is still shown.
