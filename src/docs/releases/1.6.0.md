...
All [Built-in](https://www.elastic.co/guide/en/elasticsearch/reference/current/analysis-analyzers.html) and custom analyzers are now supported. By default, only `standard` analyzer is available. You may configure the Elasticsearch service to make all the built-in and custom analyzers as needed.

Additionally, a new settings named `IndexPrefix` was added to the `OrchardCore_Elasticsearch` to allow you to define a global prefix for all index. This is helpful if cases where you want to create environment level index.
