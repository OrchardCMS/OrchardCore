...
The property name in the `SearchSettings` class was changed from `SearchProviderAreaName` to `ProviderName`. If you are using both `Lucene` and `Elasticsearch` provides at the same time, you'll need to update the search settings by setting the default search provides to one. 
