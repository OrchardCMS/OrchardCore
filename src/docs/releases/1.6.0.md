...
In order for the above settings to work for custom views, you must update your view to use the new helpers.
 1. Call `Orchard.GetLabelCssClasses()` helper to get the proper css classes for label elements.
 2. Wrap all elements that correspond to the label with an element like `div` to group them. Then use `Orchard.GetEndCssClasses()` helper to get the proper css classes for the new `div`.
...
<div class="row mb-3" asp-validation-class-for="Title">
    <!-- GetLabelCssClasses is used in the label element -->
