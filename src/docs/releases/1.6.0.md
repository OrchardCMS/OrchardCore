...
### TheAdmin theme settings

New settings were added to the `TheAdmin` there, to allow you to make the admin theme responsive based on the screen size.
...
In order for the above settings to work for custom views, you must update your view to use the helpers.
 1. Call `Orchard.GetLabelCssClasses()` helper to get the proper css classes for your label.
 2. Wrap all elements that correspond to the label with an element like `div`, and use the `Orchard.GetEndCssClasses()` helper to get the proper css classes for the new div.
...
<div class="row mb-3" asp-validation-class-for="Title">
    <label asp-for="Title" class="@Orchard.GetLabelCssClasses()">@T["Title"]</label> <!-- GetLabelCssClasses is used -->
    <div class="@Orchard.GetEndCssClasses()"> <!-- This div was added and the GetEndCssClasses() is used -->
        <input asp-for="Title" class="form-control content-preview-text content-caption-text" disabled="@(Model.Settings?.Options == TitlePartOptions.GeneratedDisabled)" autofocus="autofocus" dir="@culture.GetLanguageDirection()" />
        <span asp-validation-for="Title"></span>
        @if (!String.IsNullOrWhiteSpace(Model.Settings?.Pattern))
        {
