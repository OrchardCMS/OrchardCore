...
The size of the database identity columns is now configurable, this concerns the `Id` column of all tables, and the `DocumentId` column of all index tables.
So, all the following entities fields and map indexes fields related to these columns have been updated from an `int` to a `long` type.
