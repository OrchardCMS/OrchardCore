...
is implcitly converted to a `long` but it is now recommended to pass a `long` id parameter. The `ISession.GetAsync<T>(int[] ids, ...)` has already been
updated to `GetAsync(long[] ids, ...)`, for backward compatibility an extension method still accepting an `int[]` array has been created
but it is recommended to use the method of the updated `ISession` interface.
...
!!! note
    If you have custom implementations using the following methods, it is recommended to update them accordingly, and to change the signature of methods having
    an `int` id or a collection of `int` ids parameter so that they will accept a `long` id or a collection of `long` ids parameter.

- `ISession.GetAsync<T>(long id, string collection = null)`, updated extension method.
- `ISession.GetAsync<T>(long[] ids, string collection = null)`, updated interface method.
- `ISession.GetAsync<T>(int[] ids, string collection = null)`, not recommended extension method.
- `IContentManagerSession.RecallVersionId(long id, out ContentItem item)`, updated interface method.
- `IIndexingTaskManager.GetIndexingTasksAsync(long afterTaskId, int count)`, updated interface method.
- `IWorkflowStore.GetAsync(long id)`, updated interface method.
- `IWorkflowStore.GetAsync(IEnumerable<long> ids)`, updated interface method.
- `IWorkflowStore.GetAsync(IEnumerable<int> ids)`, not recommended extension method.
- `IWorkflowTypeStore.GetAsync(long id)`, updated interface method.
- `IWorkflowTypeStore.GetAsync(IEnumerable<long> ids)`, updated interface method.
