...
| --------- | ---- |------------ |
| `GetContentItemIdByNameAsync` | `string name` | Returns the content item id from its name. Ex: `alias:carousel`, `slug:myblog/my-blog-post` |
| `GetContentItemByNameAsync` | `string name, bool latest = false` | Loads a content item from its name, seeking the latest version or not. |
