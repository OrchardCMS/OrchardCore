...
This represents the name of the file used to store the SQLite database. By default, it is set to `OrchardCore.db`. You can change this value to specify a different file name for your SQLite database.

!!!note
    This set on the `ShellSettings` of a tenant, so it can be different for each tenant in the application.
