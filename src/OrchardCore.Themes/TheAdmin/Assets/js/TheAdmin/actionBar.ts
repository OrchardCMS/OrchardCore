...
// Manages the action bar at runtime.
//
...
(function () {
    ...
    const actionBar = document.querySelector<HTMLElement>('.action-bar-bottom');
    if (!actionBar) return;

    new ResizeObserver(() => {
        document.documentElement.style.setProperty('--oc-action-bar-bottom-height', actionBar.offsetHeight + 'px');
    }).observe(actionBar);
