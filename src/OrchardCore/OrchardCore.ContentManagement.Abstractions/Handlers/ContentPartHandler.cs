...
namespace OrchardCore.ContentManagement.Handlers
{
        ...
        where TPart : ContentPart, new()
    {
        ...
        {
            return existing is TPart texisting && building is TPart tbuilding ? VersioningAsync(context, texisting, tbuilding) : Task.CompletedTask;
        }

        Task IContentPartHandler.VersionedAsync(VersionContentContext context, ContentPart existing, ContentPart building)
        {
            return existing is TPart texisting && building is TPart tbuilding ? VersionedAsync(context, texisting, tbuilding) : Task.CompletedTask;
        }

        Task IContentPartHandler.DraftSavingAsync(SaveDraftContentContext context, ContentPart part)
        {
            return part is TPart tpart ? DraftSavingAsync(context, tpart) : Task.CompletedTask;
        }

        Task IContentPartHandler.DraftSavedAsync(SaveDraftContentContext context, ContentPart part)
        {
            return part is TPart tpart ? DraftSavedAsync(context, tpart) : Task.CompletedTask;
        }

        Task IContentPartHandler.PublishingAsync(PublishContentContext context, ContentPart part)
        {
            return part is TPart tpart ? PublishingAsync(context, tpart) : Task.CompletedTask;
        }

        Task IContentPartHandler.PublishedAsync(PublishContentContext context, ContentPart part)
        {
            return part is TPart tpart ? PublishedAsync(context, tpart) : Task.CompletedTask;
        }

        Task IContentPartHandler.UnpublishingAsync(PublishContentContext context, ContentPart part)
        {
            return part is TPart tpart ? UnpublishingAsync(context, tpart) : Task.CompletedTask;
        }

        Task IContentPartHandler.UnpublishedAsync(PublishContentContext context, ContentPart part)
        {
            return part is TPart tpart ? UnpublishedAsync(context, tpart) : Task.CompletedTask;
        }

        Task IContentPartHandler.RemovingAsync(RemoveContentContext context, ContentPart part)
        {
            return part is TPart tpart ? RemovingAsync(context, tpart) : Task.CompletedTask;
        }

        Task IContentPartHandler.RemovedAsync(RemoveContentContext context, ContentPart part)
        {
            return part is TPart tpart ? RemovedAsync(context, tpart) : Task.CompletedTask;
        }

        Task IContentPartHandler.GetContentItemAspectAsync(ContentItemAspectContext context, ContentPart part)
        {
            return part is TPart tpart ? GetContentItemAspectAsync(context, tpart) : Task.CompletedTask;
        }
        ...
        protected virtual Task ValidatingAsync(ValidateContentPartContext context, TPart part) => ValidatingAsync(context as ValidateContentContext, part);

        protected virtual Task ValidatedAsync(ValidateContentPartContext context, TPart part) => ValidatedAsync(context as ValidateContentContext, part);
    }
}
