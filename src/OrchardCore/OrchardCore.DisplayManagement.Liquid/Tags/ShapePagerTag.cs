            ...
            if (objectValue is Shape shape)
            {
                ...
                if (shape.Metadata.Type == "PagerSlim" || shape.Metadata.Type == "Pager")
                {
                    ...
                    if (expressions.HasNamed("attributes"))
                    {
                        ...
                        {
                            var values = JsonSerializer.Deserialize<Dictionary<string, string>>(attributes.ToStringValue());
                            foreach (var value in values)
                            {
                    ...
                    if (expressions.HasNamed("item_attributes"))
                    {
                        ...
                        {
                            var values = JsonSerializer.Deserialize<Dictionary<string, string>>(itemAttributes.ToStringValue());
                            foreach (var value in values)
                            {
