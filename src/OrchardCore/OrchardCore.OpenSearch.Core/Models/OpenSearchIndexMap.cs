...
namespace OrchardCore.OpenSearch.Models;

[JsonConverter(typeof(OpenSearchIndexMapConverter))]
public sealed class OpenSearchIndexMap
{
    ...
    private static void Traverse(IProperties props, string parentPath, List<string> result)
    {
        if (props is null)
        {
            return;
