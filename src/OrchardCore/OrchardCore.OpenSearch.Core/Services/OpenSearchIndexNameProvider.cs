...
public sealed class OpenSearchIndexNameProvider : IIndexNameProvider
{
    ...
    public OpenSearchIndexNameProvider(
        ...
        IOptions<OpenSearchOptions> openSearchOptions)
    {
        ...
        _prefixCacheKey = $"OpenSearchIndexesPrefix_{shellSettings.Name}";
    }
    ...
    private string GetIndexPrefix()
    {
        if (!_memoryCache.TryGetValue<string>(_prefixCacheKey, out var value))
        {
            ...
            if (!string.IsNullOrWhiteSpace(_openSearchOptions.IndexPrefix))
            {
                parts.Add(_openSearchOptions.IndexPrefix.ToLowerInvariant());
            }
    ...
    /// <summary>
    /// Makes sure that the index names are compliant with OpenSearch specifications.
    /// </summary>
