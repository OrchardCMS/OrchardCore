    ...
    public class CultureDictionary : IEnumerable<CultureDictionaryRecord>
    {
        ...
        public string this[CultureDictionaryRecordKey key, int? count]
        {
            get
            {
                ...
                {
                    throw new PluralFormNotFoundException(
                        $"Plural form '{pluralForm}' doesn't exist for the key '{key}' in the '{CultureName}' culture.",
                        new PluralForm(key, pluralForm, CultureInfo.GetCultureInfo(CultureName))
                    );
                }
