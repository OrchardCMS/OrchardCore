...
public static class JNode
{
    ...
    public static T? Value<T>(this JsonNode? node) =>
        node is JsonValue jsonValue && jsonValue.TryGetValue<T>(out var value) ? value : default;

    /// <summary>
    /// Gets the value of the specified type from the specified property of this <see cref="JsonNode"/>.
    /// </summary>
    public static T? Value<T>(this JsonNode? node, string name) => node is not null ? node[name].Value<T>() : default;
