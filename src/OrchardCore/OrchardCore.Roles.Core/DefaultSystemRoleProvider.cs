...
public sealed class DefaultSystemRoleProvider : ISystemRoleProvider
{
        ...
        IOptions<SystemRoleOptions> options)
    {
        _adminRole = new Role
        {
            RoleName = GetAdminRole(shellSettings, options),
            RoleDescription = S["A system role that grants all permissions to the assigned users."],
        };

        _systemRoles = new Dictionary<string, IRole>(options.Value.AdditionalSystemRoles)
        {
    ...
    private static string GetAdminRole(ShellSettings shellSettings, IOptions<SystemRoleOptions> options)
    {
