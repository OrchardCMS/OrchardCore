...
public readonly struct GroupingMetadata : IEquatable<GroupingMetadata>
{
    ...
    /// <summary>
    /// Returns the length this grouping would occupy in a location string (excluding the delimiter).
    /// </summary>
    internal int GetLocationStringLength()
    {
        if (!HasValue)
        {
            return 0;
    ...
    /// <summary>
    /// Writes this grouping to a span at the specified offset. Returns the number of characters written.
    /// </summary>
    internal int WriteToSpan(Span<char> span, int offset)
    {
        if (!HasValue)
        {
            return 0;
        ...
        Name.AsSpan().CopyTo(span[offset..]);
        offset += Name.Length;

        if (!string.IsNullOrEmpty(Width))
        {
            span[offset++] = '_';
            Width.AsSpan().CopyTo(span[offset..]);
            offset += Width.Length;
        }
        ...
        {
            span[offset++] = ';';
            Position.AsSpan().CopyTo(span[offset..]);
