...
namespace Microsoft.Extensions.DependencyInjection
{
    public static class ServiceCollectionExtensions
    {
        ...
        private static readonly Type[] _httpClientTypesToIsolate = new ServiceCollection()
            ...
            .Select(sd => sd.GetImplementationType())
            .Except(new ServiceCollection().AddLogging().Where(sd => sd.Lifetime == ServiceLifetime.Singleton).Select(sd => sd.GetImplementationType()))
            .ToArray();
        ...
        public static OrchardCoreBuilder AddOrchardCore(this IServiceCollection services)
        {
            ...
            var builder = services.LastOrDefault(d => d.ServiceType == typeof(OrchardCoreBuilder))?.ImplementationInstance as OrchardCoreBuilder;
        ...
        private static void AddShellServices(OrchardCoreBuilder builder)
        {
            ...
            services.AddTransient(sp => new ShellFeature(sp.GetRequiredService<IHostEnvironment>().ApplicationName, alwaysEnabled: true));

            // Registers the application default feature.
            services.AddTransient(sp => new ShellFeature(Application.DefaultFeatureId, alwaysEnabled: true));
        ...
        private static void AddStaticFiles(OrchardCoreBuilder builder)
        {
            builder.ConfigureServices(services =>
            {
                services.AddSingleton<IModuleStaticFileProvider>(serviceProvider =>
                {
                    ...
                    {
                        var fileProviders = new List<IStaticFileProvider> { new ModuleProjectStaticFileProvider(appContext), new ModuleEmbeddedStaticFileProvider(appContext) };
            ...
            builder.Configure(
                (app, routes, serviceProvider) =>
                {
                    ...
                    var cacheControl = shellConfiguration.GetValue(
                        "StaticFileOptions:CacheControl",
                        $"public, max-age={TimeSpan.FromDays(30).TotalSeconds}, s-maxage={TimeSpan.FromDays(365.25).TotalSeconds}"
                    );
        ...
        private static void AddMetrics(OrchardCoreBuilder builder)
        {
            ...
            builder.ConfigureServices(
                collection =>
                {
                    ...
                    var descriptorsToRemove = collection.Where(sd => sd is ClonedSingletonDescriptor && _metricsTypesToIsolate.Contains(sd.GetImplementationType())).ToArray();
            ...
            builder.ConfigureServices(
                collection =>
                {
                    ...
                    var descriptorsToRemove = collection
                        .Where(sd =>
                            (sd is ClonedSingletonDescriptor || sd.ServiceType == typeof(IConfigureOptions<RouteOptions>))
            ...
            builder.ConfigureServices(
                collection =>
                {
                    ...
                    var configurationDescriptorsToRemove = collection
                        .Where(sd => sd.ServiceType.IsGenericType && sd.ServiceType.GenericTypeArguments.Contains(typeof(HttpClientFactoryOptions)))
                        .ToArray();

                    // Retrieve all descriptors to remove.
                    var descriptorsToRemove = collection
                        .Where(sd => sd is ClonedSingletonDescriptor && _httpClientTypesToIsolate.Contains(sd.GetImplementationType()))
                ...
                collection =>
                {
                    ...
                    var descriptorsToRemove = collection
                        .Where(sd =>
                            ...
                            && (sd.ServiceType == typeof(IActionDescriptorCollectionProvider) || sd.ServiceType == typeof(IApiDescriptionGroupCollectionProvider))
                        )
                        .ToArray();
            ...
            builder.ConfigureServices(
                (services, serviceProvider) =>
                {
                    ...
                    var directory = new DirectoryInfo(
                        Path.Combine(options.Value.ShellsApplicationDataPath, options.Value.ShellsContainerName, settings.Name, "DataProtection-Keys")
                    );
