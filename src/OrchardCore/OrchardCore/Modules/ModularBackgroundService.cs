...
namespace OrchardCore.Modules
{
    internal class ModularBackgroundService : BackgroundService
    {
        ...
        {
            await GetShellsToRun(runningShells)
                .ForEachAsync(async tenant =>
                {
        ...
        private async Task UpdateAsync((string Tenant, long UtcTicks)[] previousShells, (string Tenant, long UtcTicks)[] runningShells, CancellationToken stoppingToken)
        {
            var referenceTime = DateTime.UtcNow;

            await GetShellsToUpdate(previousShells, runningShells)
                .ForEachAsync(async tenant =>
                {
                    if (stoppingToken.IsCancellationRequested)
                    {
        ...
        private (string Tenant, long UtcTicks)[] GetRunningShells() =>
            _shellHost
                .ListShellContexts()
        ...
        {
            var tenantsToRun = _schedulers.Where(scheduler => scheduler.Value.CanRun()).Select(scheduler => scheduler.Value.Tenant).Distinct().ToArray();

            return shells.Select(shell => shell.Tenant).Where(tenant => tenantsToRun.Contains(tenant)).ToArray();
        }
        ...
        private string[] GetShellsToUpdate((string Tenant, long UtcTicks)[] previousShells, (string Tenant, long UtcTicks)[] runningShells)
        {
            ...
            {
                if (_shellHost.TryGetShellContext(tenant, out var existing) && existing.UtcTicks == utcTicks && !existing.Released)
                {
                    continue;
                }
        ...
        private BackgroundTaskScheduler[] GetSchedulersToRun(string tenant) =>
            _schedulers.Where(scheduler => scheduler.Value.Tenant == tenant && scheduler.Value.CanRun()).Select(scheduler => scheduler.Value).ToArray();
