...
namespace OrchardCore.Tenant.Builders
{
    public class TenantContainerFactory : ITenantContainerFactory
    {
        ...
        {
            services.TryAddScoped<ITenantStateUpdater, TenantStateUpdater>();
            services.TryAddScoped<ITenantStateManager, NullTenantStateManager>();
            services.AddScoped<TenantStateCoordinator>();
            services.AddScoped<ITenantDescriptorManagerEventHandler>(sp => sp.GetRequiredService<TenantStateCoordinator>());
        }

        public IServiceProvider CreateContainer(TenantSettings settings, TenantBlueprint blueprint)
        {
            ...
            // Make tenant settings available to the modules
            ...
            foreach (var startup in moduleServiceProvider.GetServices<OrchardCore.Modules.IStartup>())
            {
            ...
            var typeFeatureProvider = tenantServiceProvider.GetRequiredService<ITypeFeatureProvider>();
