...
namespace OrchardCore.Filters.Core
{
    public abstract class FilterEngineModelBinder<T> : IModelBinder where T : class
    {
        protected IQueryParser<T> _parser;

        protected FilterEngineModelBinder(IQueryParser<T> parser)
        {
            _parser = parser;
        }
