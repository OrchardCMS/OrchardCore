...
public class AzureCognitiveSearchAuthorizationHandler(IServiceProvider serviceProvider) : AuthorizationHandler<PermissionRequirement>
{
    ...
    protected override async Task HandleRequirementAsync(AuthorizationHandlerContext context, PermissionRequirement requirement)
    {
        ...
        if (AzureCognitiveSearchService.Key != parameters.ServiceName)
        {
            // Only validate if AzureCognitiveSearch is requested.
            return;
        }
        ...
        if (!string.IsNullOrEmpty(indexName))
        {
            ...
            var permission = AzureCognitiveSearchIndexPermissionHelper.GetElasticIndexPermission(indexName);
    ...
    private async Task<string> GetIndexNameAsync(SearchPermissionParameters parameters)
    {
        ...
        return siteSettings.As<AzureCognitiveSearchSettings>().SearchIndex;
    }
}
