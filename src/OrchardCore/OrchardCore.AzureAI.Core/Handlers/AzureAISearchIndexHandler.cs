...
namespace OrchardCore.AzureAI.Handlers;
...
public sealed class AzureAISearchIndexHandler : IndexProfileHandlerBase
{
    ...
    private static Task PopulateAsync(IndexProfile indexProfile, JsonNode data)
    {
        ...
        if (data[nameof(metadata.VectorSearch)] is { } vectorSearch)
        {
            metadata.VectorSearch = vectorSearch.ToObject<AzureAISearchVectorSearchOptions>();
        }
