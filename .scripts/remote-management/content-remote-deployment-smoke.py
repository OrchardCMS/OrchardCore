...
with tempfile.TemporaryDirectory(prefix='content-remote-cli-', dir=state_path.parent) as config_home:
    for path in [source_path, target_path]:
        select(path)
        for feature in ['OrchardCore.Deployment.Remote', 'OrchardCore.Contents.Deployment.Download',
        ...
        pomi('context', 'add', name + state_path.parent.name, base, '--current')
    ...
    client = pomi('deployment', 'remote-clients', 'create', body={'clientName': name, 'apiKey': key})['resource']
    target_url = base + 'OrchardCore.Deployment.Remote/ImportRemoteInstance/Import'
    select(source_path)
    item = pomi('content', 'items', 'save', body={'ContentType': name, 'TitlePart': {'Title': 'Published source'}})
    item_id = item['ContentItemId']
    pomi('content', 'items', 'update-draft', item_id, body={'TitlePart': {'Title': 'Private latest source'}})
    destination = pomi('deployment', 'remote-instances', 'create', body={'name': name, 'url': target_url, 'clientName': name, 'apiKey': key})['resource']
    plan = pomi('deployment', 'plans', 'create', body={'name': name})['plan']
    pomi('deployment', 'plans', 'steps', 'add', str(plan['id']), body={'id': 'content', 'type': 'ExportContentToDeploymentTargetDeploymentStep',
        'values': {'contentItemIds': [item_id], 'latest': True}})
    request('api/deployment/targets/' + destination['id'] + '/send', 'POST', {'planId': plan['id']}, client='cli-remote-export', status=403)
    select(target_path)
    request('api/content/' + item_id + '/export?latest=true', status=404)
    select(source_path)
    assert pomi('deployment', 'targets', 'send', destination['id'], '--force', body={'planId': plan['id']})['succeeded']
    select(target_path)
    imported = pomi('content', 'export', item_id, '--latest', 'true')
    assert imported['TitlePart']['Title'] == 'Private latest source'
    assert not imported['Published']
    request('api/content/' + item_id + '/export', status=404)
    assert tool('content_export', {'path': {'contentItemId': item_id}, 'query': {'latest': True}})['ContentItemId'] == item_id
    assert pomi('deployment', 'remote-clients', 'delete', client['id'], '--force')['changed']
    select(source_path)
    assert pomi('deployment', 'remote-instances', 'delete', destination['id'], '--force')['changed']
    print('PASS: selected draft delivered to another tenant; per-item denial prevents send; latest/published semantics survive import through shared deployment services', flush=True)
