...
| [Contents](../../src/OrchardCore.Modules/OrchardCore.Contents/Manifest.cs) / `OrchardCore.Contents.Deployment.ExportContentToDeploymentTarget` | Shared explicit content selection and JSON export | `content export`, `deployment plans steps` | Admin download and deployment sources share version selection, serialization and per-item authorization; explicit IDs support queued archives and remote targets. | B08 |
| [Contents](../../src/OrchardCore.Modules/OrchardCore.Contents/Manifest.cs) / `OrchardCore.Contents.Deployment.Download` | Shared explicit content selection and JSON export | `content export`, `deployment plans steps` | Admin download and deployment sources share version selection, serialization and per-item authorization; explicit IDs support queued archives and remote targets. | B08 |
