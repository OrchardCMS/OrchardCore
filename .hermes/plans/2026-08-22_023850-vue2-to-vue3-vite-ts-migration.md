...
**Status: both open sub-decisions confirmed with the user (2026-08-22).**

**Design:**
- New file: `.scripts/bloom/components/options-table-editor.ts` - **NOT a `.vue` SFC.** Confirmed with the user: every existing bloom component is a plain `.ts` file using `Vue.createApp({ ... })` (Options API, template as a string) against the shared global `vuejs` UMD resource - see `content-type-picker.ts` (Task 2) and `translation-editor.ts` for the established pattern. No `.vue` SFC exists anywhere in this repo outside the fully independent `media-picker`/`media-gallery` Vite apps (which have their own complete Vite + `@vitejs/plugin-vue` build, entirely separate from bloom's `vue-tsc --noEmit`-only workspace). Introducing a real SFC into `bloom/` would require adding SFC compilation to a workspace that has never had it, and confirming every `parcel`-built consumer module can resolve a `.vue` import - unjustified new build-tooling risk for what a plain `.ts` component (template as a string, same as every other bloom component) already solves identically. A generic component export (e.g. `createOptionsTableEditorApp(options)`) taking:
  - `rows: Record<string, string>[]` - the row array (bound via `data()`, not a v-model prop, since this is a root-mounted app per consumer instance, not a reusable child component instantiated multiple times per page)
  ...
  - `defaultColumn?: { key: string; label: string; mode: "radio" | "checkbox"; radioGroupName?: string }` - optional "mark as default" column. **Confirmed with the user: both existing UI modes are preserved as-is via this `mode` field, not standardized on one** - `TextFieldPredefinedListEditorSettings.Edit.cshtml` keeps its radio (`mode: "radio"`, needs `radioGroupName` since Vue 3's `v-model` on a radio group requires an explicit shared name, same as the current form-field-name binding), `MultiTextFieldSettings.Edit.cshtml` keeps its per-row checkbox (`mode: "checkbox"`, `option.default`); OpenId/Seo omit `defaultColumn` entirely (unchanged).
  - `addLabel: string` - the "Add a ..." button text (varies per consumer: "option"/"parameter"/"custom meta tag")
  - a companion sub-component (still `.ts`, template-as-string, registered locally via `components: { ... }` on the root `createApp` call) for the raw-JSON-textarea fallback editor, reusing the same generic row shape
...
**Files:**
- Create: `.scripts/bloom/components/options-table-editor.ts`
...
- Modify views (extract X-Templates, since the template now lives in the shared `.ts` component as a string, not inline `<script type="text/x-template">`): `TextFieldPredefinedListEditorSettings.Edit.cshtml`, `MultiTextFieldSettings.Edit.cshtml`, `OpenIdClientSettings.Edit.cshtml`, `SeoMetaPart.Edit.cshtml` - each keeps only its mount-point `<div data-*="...">` and drops the `<script type="text/x-template">` blocks entirely.
...
**Steps:**
1. ~~Get the radio-vs-checkbox decision confirmed with the user~~ DONE - both modes preserved via `defaultColumn.mode`, see Design above.
2. Read `.scripts/bloom/components/` and `.scripts/bloom/helpers/` directory listings in full to match existing naming/structure conventions exactly (do not invent a new pattern shape for this one component) - already done for this decision; re-confirm exact naming conventions (camelCase vs kebab-case file names, export shape) against 2-3 existing components before writing new files.
3. Build `options-table-editor.ts` with the four consumers' actual column configs as the acceptance test (all four must render pixel-identical Bootstrap-classed markup to their current X-Templates - this is a mechanical port, not a redesign).
4. Build the modal sub-part (JSON textarea fallback editor) as a locally-registered sub-component within the same file (Options API `components: {}`, template-as-string) - do not introduce a `.vue` SFC for this either, per the Design decision above.
...
8. `options-table-editor.ts` lives in the `@orchardcore/bloom` workspace like every other shared component/helper - no new build wiring needed, each consumer module already imports from `@orchardcore/bloom/...` via its own `parcel`/Vite build the same way `content-type-picker.ts` already imports `@orchardcore/bloom/helpers/evalScripts` (Task 2 precedent).
9. Build, smoke-test all 4 admin views manually (TextField predefined-list settings, MultiTextField settings, OpenId client parameters, Seo custom meta tags) - table add/remove/reorder, default-marking (radio for TextFieldPredefinedListEditorSettings, checkbox for MultiTextFieldSettings, per the preserved-modes decision), JSON-modal round-trip, and for Seo specifically the AJAX-widget-injection case.
...
**Objective:** The new `options-table-editor.ts` built in Task 3 still needs SOME reorder mechanism for its draggable rows - resolve this using Task 1's decision (vuedraggable@next vs. hand-rolled SortableJS) rather than deciding it independently mid-Task-3. If Task 1 hasn't run yet, run it first.
