...
For a visible trail, the `<breadcrumb>` tag helper produces a `Breadcrumb` shape holding one `BreadcrumbItem` shape per final node. It collects ancestor children, invokes `IBreadcrumbProvider` implementations in DI registration order, stably sorts the ancestors, then unconditionally appends the parent's explicit `title` as the final/current node with `Id = "Title"`. Both shapes are ordinary display-managed shapes, so a theme overrides their rendering by supplying a `.cshtml` whose name matches an alternate. The display engine tries alternates **most specific first**; the last one in each list below is the most specific.
...
After providers finish, the parent helper orders ancestors stably with `FlatPositionComparer`; equal positions retain their post-provider list order. It appends the explicit title node afterward, even if all ancestors were cleared or an ancestor has position `end`. Only this appended node is current and never linked; its `<li>` gets `active` and `aria-current="page"`. The last sorted ancestor remains an ancestor. Ancestors render an `<a>` only when they have an authorized `Href`; denied permissions, `link-enabled="false"`, or no link target leave plain text. Overrides should respect `Href` rather than rebuilding links from route values.
