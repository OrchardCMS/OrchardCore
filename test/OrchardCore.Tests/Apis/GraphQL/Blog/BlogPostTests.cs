...
namespace OrchardCore.Tests.Apis.GraphQL
{
    public class BlogPostTests
    {
        ...
        public async Task ShouldListAllBlogs()
        {
            ...
            Assert.Single(result["data"]["blog"].AsArray().Where(node => node["contentItemId"].ToString() == context.BlogContentItemId));
        }
        ...
        public async Task ShouldQueryByBlogPostAutoroutePart()
        {
            ...
            var blogPostContentItemId1 = await context.CreateContentItem(
                "BlogPost",
                builder =>
                {
                    builder.DisplayText = "Some sorta blogpost!";

                    builder.Weld(new AutoroutePart { Path = "Path1", });

                    builder.Weld(new ContainedPart { ListContentItemId = context.BlogContentItemId, });
                }
            );

            var blogPostContentItemId2 = await context.CreateContentItem(
                "BlogPost",
                builder =>
                {
                    builder.DisplayText = "Some sorta other blogpost!";

                    builder.Weld(new AutoroutePart { Path = "Path2", });
            ...
            Assert.Equal("Some sorta blogpost!", result["data"]["blogPost"][0]["displayText"].ToString());
        }
        ...
        public async Task WhenThePartHasTheSameNameAsTheContentTypeShouldCollapseFieldsToContentType()
        {
            ...
            Assert.Equal("Problems look mighty small from 150 miles up", result["data"]["blogPost"][0]["subtitle"].ToString());
        }
        ...
        public async Task WhenCreatingABlogPostShouldBeAbleToPopulateField()
        {
            ...
            var blogPostContentItemId = await context.CreateContentItem(
                ...
                builder =>
                {
                    ...
                    builder.Weld("BlogPost", new ContentPart());

                    builder.Alter<ContentPart>(
                        "BlogPost",
                        (cp) =>
                        {
            ...
            var result = await context.GraphQLClient.Content.Query(
                ...
                builder =>
                {
                    builder.WithQueryStringArgument("where", "ContentItemId", blogPostContentItemId);

                    builder.WithField("Subtitle");
                }
            );

            Assert.Equal("Hey - Is this working!?!?!?!?", result["data"]["blogPost"][0]["subtitle"].ToString());
        }
        ...
        public async Task ShouldQueryByStatus()
        {
            ...
            var draft = await context.CreateContentItem(
                "BlogPost",
                builder =>
                {
            ...
            var result = await context.GraphQLClient.Content.Query("blogPost(status: PUBLISHED) { displayText, published }");

            Assert.Single(result["data"]["blogPost"].AsArray());
            Assert.True(result["data"]["blogPost"][0]["published"].Value<bool>());

            result = await context.GraphQLClient.Content.Query("blogPost(status: DRAFT) { displayText, published }");

            Assert.Single(result["data"]["blogPost"].AsArray());
            Assert.False(result["data"]["blogPost"][0]["published"].Value<bool>());

            result = await context.GraphQLClient.Content.Query("blogPost(status: LATEST) { displayText, published }");
        ...
        {
            using var context = new SiteContext().WithPermissionsContext(new PermissionsContext { UsePermissionsContext = true });
        ...
        public async Task ShouldReturnBlogsWithViewBlogContentPermission()
        {
            using var context = new SiteContext().WithPermissionsContext(
                new PermissionsContext { UsePermissionsContext = true, AuthorizedPermissions = new[] { GraphQLApi.Permissions.ExecuteGraphQL, Contents.Permissions.ViewContent, }, }
            );
        ...
        public async Task ShouldNotReturnBlogsWithViewOwnBlogContentPermission()
        {
            using var context = new SiteContext().WithPermissionsContext(
                new PermissionsContext
                {
                    ...
                    AuthorizedPermissions = new[] { GraphQLApi.Permissions.ExecuteGraphQL, Contents.Permissions.ViewOwnContent, },
                }
            );
        ...
        {
            using var context = new SiteContext().WithPermissionsContext(
                new PermissionsContext { UsePermissionsContext = true, AuthorizedPermissions = new[] { GraphQLApi.Permissions.ExecuteGraphQL, }, }
            );
