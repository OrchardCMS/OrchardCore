...
namespace OrchardCore.Tests.Modules.OrchardCore.Twitter
{
    public class TwitterClientTests
    {
        ...
        public TwitterClientTests()
        {
            ...
            var signinSettings = new TwitterSigninSettings { CallbackPath = null, };

            _mockDataProtectionProvider = Mock.Of<IDataProtectionProvider>(dpp => dpp.CreateProtector(It.IsAny<string>()) == fakeDataProtector);
        ...
        public async Task HttpRequestMessageShouldHaveCorrectSignedOAuthHeader()
        {
            ...
            _mockFakeHttpMessageHandler
                .Setup(c => c.Send(It.IsAny<HttpRequestMessage>()))
                .Returns((HttpRequestMessage request) => new HttpResponseMessage(HttpStatusCode.OK) { Content = new StringContent(UpdateStatusResponse), })
                .Callback<HttpRequestMessage>((msg) => message = msg);

            var mockTwitterClient = new Mock<TwitterClient>(new HttpClient(_mockFakeHttpMessageHandler.Object), Mock.Of<ILogger<TwitterClient>>()) { CallBase = true, };

            _mockHttpMessageHandler.Setup(c => c.GetNonce()).Returns(() => "kYjzVBB8Y0ZFabxSWbWovY3uYSQ2pTgmZeNu2VS4cg");
