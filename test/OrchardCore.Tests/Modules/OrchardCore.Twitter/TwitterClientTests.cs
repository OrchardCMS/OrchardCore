...
namespace OrchardCore.Tests.Modules.OrchardCore.Twitter
{
    public class TwitterClientTests
    {
        ...
        Mock<FakeHttpMessageHandler> moqHttpMessageHandler;
        public TwitterClientTests()
        {
            moqHttpMessageHandler = new Mock<FakeHttpMessageHandler>() { CallBase = true };

        }

        [Fact]
        public async Task UpdateStatusShouldReturnJson()
        {
            HttpRequestMessage message = null;
            moqHttpMessageHandler.Setup(c => c.Send(
                    It.IsAny<HttpRequestMessage>())
                ).Returns((HttpRequestMessage request) =>
                    new HttpResponseMessage(System.Net.HttpStatusCode.OK) { Content = new StringContent(updateStatusResponse) }
            ...
            var moqTwitterClient = new Mock<TwitterClient>(new HttpClient(moqHttpMessageHandler.Object),
                Mock.Of<IClock>(i => i.UtcNow == new DateTime(ticks)),
                Mock.Of<ILogger<TwitterClient>>(),
                Mock.Of<IOptionsMonitor<TwitterOptions>>(s => s.CurrentValue == new TwitterOptions()
                {
                    ConsumerKey = "xvz1evFS4wEEPTGEFPHBog",
                    ConsumerSecret = "kAcSOqF21Fu85e7zjz7ZN2U4ZRhfV3WpwPAoE3Z7kBw"
                }),
                Mock.Of<IOptionsMonitor<TwitterClientOptions>>(s => s.CurrentValue == new TwitterClientOptions()
                {
            ...
            moqTwitterClient.Setup(c => c.GetNonce())
                .Returns(() => "kYjzVBB8Y0ZFabxSWbWovY3uYSQ2pTgmZeNu2VS4cg");

            var result = await moqTwitterClient.Object.UpdateStatus("Hello Ladies + Gentlemen, a signed OAuth request!", "include_entities", "true");

            Assert.NotNull(message);
            Assert.True(message.Content.Headers.Contains("Authentication"));
            Assert.Equal(expectedOauthHeader, message.Content.Headers.GetValues("Authentication").FirstOrDefault());
        }
