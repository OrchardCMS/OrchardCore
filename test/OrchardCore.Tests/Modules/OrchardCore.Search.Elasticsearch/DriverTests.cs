        ...
        await (await CreateShellContext().CreateScopeAsync()).UsingAsync(async scope =>
        {
            ...
            var contentDefinition =
                BuildContentPartWithField(field => field
                            ...
                            .WithSettings(new ContentPickerFieldElasticEditorSettings
                            {
                                Index = "testIndex",
                                Indices = ["idx1", "idx2", "testIndex"]
                            }));
            var elasticService = new ElasticIndexSettingsService();
            var shapeResult = await GetShapeResult(contentDefinition, new ContentPickerFieldElasticEditorSettingsDriver(elasticService));
            var settings = (ContentPickerFieldElasticEditorSettings)shapeResult.Shape;
