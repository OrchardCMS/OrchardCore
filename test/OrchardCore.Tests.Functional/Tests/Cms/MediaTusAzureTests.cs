...
public sealed class MediaTusAzureTests : IClassFixture<SaasFixture>, IAsyncLifetime
{
    ...
    public MediaTusAzureTests(SaasFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    {
        _tenant = TestUtils.GenerateTenantInfo("MediaTusAzure");
    ...
    [AzuriteFact]
    public async Task ShouldUploadFileViaTusWithAzureTempStore()
    {
        var page = await _fixture.CreatePageAsync();
        await AuthHelper.LoginAsync(page, $"/{_tenant.Prefix}");
        await MediaHelper.NavigateToMediaAsync(page, $"/{_tenant.Prefix}");

        var filePath = MediaHelper.GenerateTestFile("tus-azure-upload.jpg", SmallFileSize);
        await MediaHelper.UploadFileAsync(page, filePath);
        await MediaHelper.ExpectFileInLibraryAsync(page, "tus-azure-upload.jpg");

        await page.CloseAsync();
    }

    [AzuriteFact]
    public async Task ShouldPauseAndResumeWithAzureTempStore()
    {
        var page = await _fixture.CreatePageAsync();
        await AuthHelper.LoginAsync(page, $"/{_tenant.Prefix}");
        await MediaHelper.NavigateToMediaAsync(page, $"/{_tenant.Prefix}");
        ...
        var filePath = MediaHelper.GenerateTestFile("tus-azure-pause.jpg", LargeFileSize);
        await MediaHelper.UploadFileAsync(page, filePath);
        ...
        var fileRow = container.Locator(".upload-toast-item").Filter(new() { HasText = "tus-azure-pause.jpg" });
        ...
        await MediaHelper.NavigateToMediaAsync(page, $"/{_tenant.Prefix}");
        var verifyFile = MediaHelper.GenerateTestFile("tus-azure-verify.jpg", SmallFileSize);
        await MediaHelper.UploadFileAsync(page, verifyFile);
        await MediaHelper.ExpectFileInLibraryAsync(page, "tus-azure-verify.jpg");
