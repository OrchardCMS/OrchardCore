...
import fs from 'fs';
import { deleteAppData, copyMigrationsRecipe, copyRecipe, hostApp, waitForReady } from './helpers/app-lifecycle';
...
async function globalSetup(): Promise<void> {
    ...
    // Copy test recipes for CMS tests (after clean so they're not deleted)
    if (!isMvc) {
        copyMigrationsRecipe(fixturesDir, appDir);
        copyRecipe(fixturesDir, appDir, 'media-tus.recipe.json');
    }
